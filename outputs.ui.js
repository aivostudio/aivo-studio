(function () {
  ...
  function ensureMount() {
    ...
    // ✅ doğru yer: header'dan hemen sonra
...
.out-empty{
  ...
  function render() {
    ...
    const active =
      state.tab === "video" ? videos : state.tab === "audio" ? audios : images;

    // Search filter (title/sub/status)
    const q = (state.q || "").trim().toLowerCase();
    const filtered = q
      ? active.filter((x) => {
          const badge =
            x.status === "ready" ? "hazır" : x.status === "error" ? "hata" : "sırada";
  ...
  function cardHTML(item) {
    const badgeText =
      item.status === "ready" ? "Hazır" : item.status === "error" ? "Hata" : "Sırada";

    const badgeCls =
      item.status === "ready" ? "is-ready" : item.status === "error" ? "is-error" : "is-queued";
  ...
  function ensureModal() {
    ...
    // Minimal inline styles (avoid dependency)
    m.style.position = "fixed";
    m.style.inset = "0";
    m.style.zIndex = "9999";

    m.innerHTML = `
      <div class="aivo-prev__bg" data-close="1" style="position:absolute;inset:0;background:rgba(0,0,0,.65);backdrop-filter:blur(10px);"></div>
      <div class="aivo-prev__card" style="position:relative;max-width:820px;margin:6vh auto 0;border-radius:18px;overflow:hidden;border:1px solid rgba(255,255,255,.10);background:rgba(12,14,24,.85);box-shadow:0 18px 60px rgba(0,0,0,.6);">
        <button class="aivo-prev__x" data-close="1" style="position:absolute;top:10px;right:10px;z-index:2;width:38px;height:38px;border-radius:12px;border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.06);color:#fff;cursor:pointer;">✕</button>
        <div class="aivo-prev__media" id="aivoPrevMedia" style="padding:18px;"></div>
        <div class="aivo-prev__bar" style="display:flex;gap:12px;align-items:center;justify-content:space-between;padding:14px 18px;border-top:1px solid rgba(255,255,255,.08);">
          <div class="aivo-prev__title" id="aivoPrevTitle" style="font-weight:800;color:#fff;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">Çıktı</div>
          <a class="aivo-prev__dl" id="aivoPrevDl" href="#" download
