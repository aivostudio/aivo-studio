(function () {
  ...
  // ✅ PROMPT: ilk 2 kelimeyi başlık yap
  ...
  function displayTitle(it) {
    // Öncelik: prompt ilk 2 kelime → title → Kapak
  ...
  function ensureStyles() {
    ...
    s.textContent = `
      ...
      .cpCard{
        /* ✅ mod.cover.panel.css'deki font-size: var(--cp-title-size) override'ı için */
        --cp-title-size: 13px;
      ...
      /* bottom title */
      .cpBottom{
        padding: 10px 12px 12px; /* biraz sıkı */
        display:flex;
        align-items:center;
        gap:10px;
      }

      /* ✅ başlık küçük + gallery hissi */
      .cpName{
  ...
  function render(host) {
    ...
    grid.innerHTML = state.items.map(it => {
      ...
      // ✅ başlık: prompt ilk 2 kelime
      const name = displayTitle(it);
      ...
      return `
        <div class="cpCard" data-id="${esc(it.id)}" tabindex="0">
          ...
          <div class="cpBottom">
            <div class="cpName" title="${esc(it?.prompt || name)}">${esc(name)}</div>
          </div>
        </div>
      `;
    }).join("");
  ...
  function attachPPE(host) {
    ...
    PPE.onOutput = (job, out) => {
      ...
      upsertItem({
        ...
        status: "COMPLETED",
        url: out.url,
        // ✅ prompt/title gelirse sakla ki ilk 2 kelime çıksın
        prompt: out?.meta?.prompt || job?.prompt || "",
