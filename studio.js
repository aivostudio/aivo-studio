...
/* =========================================================
   🎛️ VIDEO UI COST LABEL SYNC (10/14) — ULTRA SAFARI SAFE
...
(function(){
  ...
  function findVideoRoot(){
    return document.querySelector(".page-video")
      || document.querySelector("#pageVideo")
      || document.querySelector("[data-page='video']")
  ...
  function sync(){
    ...
    // 1) Video butonu yazısı
    var btn = document.getElementById("videoGenerateTextBtn") || document.querySelector("button[data-generate='video']");
    if (btn) {
      var tx = (btn.textContent || "");
      if (tx.toLowerCase().indexOf("video oluştur") !== -1) {
        setText(btn, "🎞️ Video Oluştur (" + cost + " Kredi)");
      }
      btn.setAttribute("data-credit-cost", String(cost));
    }

    // 2) "15 Kredi" rozetini bul ve güncelle
    var root = findVideoRoot();
    ...
    for (var i=0; i<nodes.length; i++){
      ...
      if (t.toLowerCase().indexOf("kredi") === -1) continue;
    ...
    if (best) setText(best, cost + " Kredi");
  }
  ...
  // DOM geç gelirse (MutationObserver bazı Safari sürümlerinde sorun çıkarmaz ama try/catch)
  try{
    var mo = new MutationObserver(function(){ try{ sync(); } catch(e){} });
