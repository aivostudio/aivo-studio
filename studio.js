...
if (shouldPlayerBeAllowed()) gpShow();
  ...
  // ================= AIVO DOCK PLAYER =================
document.addEventListener("DOMContentLoaded", () => {
  const player = document.querySelector(".aivo-dock-player");
  if (!player) return;

  const audio = player.querySelector(".dock-audio");
  const btnPlay = player.querySelector(".dock-play");
  const btnClose = player.querySelector(".dock-close");
  const prog = player.querySelector(".dock-progress");
  const fill = player.querySelector(".progress-fill");
  const knob = player.querySelector(".progress-knob");
  const tCur = player.querySelector(".time-current");
  const tTot = player.querySelector(".time-total");
  const vol = player.querySelector(".vol-range");

  // Varsayılan: ses yoksa bile UI çalışsın. İstersen buraya demo mp3 yolu yaz:
  ...
  const syncTime = () => {
    ...
    tTot.textContent = fmt(audio.duration || 0);
  ...
  btnPlay.addEventListener("click", async () => {
    try{
      if (!audio.src) {
        // Track bağlanmadıysa “demo” gibi davran: sadece animasyon
  ...
  prog.addEventListener("pointercancel", () => dragging = false);
