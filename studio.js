...
document.addEventListener("click", function (e) {
  try {
    ...
    /* ===============================
       🎯 GERÇEK VIDEO GENERATE BUTONU
       =============================== */
    var btn = t.closest
      ? t.closest("#videoGenerateTextBtn, button[data-generate='video']")
      : null;
    ...
    function isVideoAudioEnabled() {
      ...
      // 2) UI switch / class / aria kontrolü
      ...
      var row =
        title.closest(".audio-row, .audio-card, .audio-box, section, div") ||
        title.parentElement;
      ...
      var sw =
        ...
        row.querySelector(".switch, .toggle, .slider, .pill");

      if (!sw) return false;

      var aria = sw.getAttribute("aria-checked");
      if (aria === "true") return true;
      if (aria === "false") return false;

      var ds = sw.getAttribute("data-state");
      if (ds === "on" || ds === "checked" || ds === "true") return true;
      if (ds === "off" || ds === "unchecked" || ds === "false") return false;

      var cls = (sw.className || "").toLowerCase();
      if (cls.includes("active") || cls.includes("on")) return true;
    ...
    /* ===============================
       💳 MALİYET HESABI
       =============================== */
    var cost = isVideoAudioEnabled() ? 14 : 10;
    ...
    if (!ok) {
      ...
      if (typeof openPricingIfPossible === "function") {
        ...
        document.querySelector(".btn-credit-buy, [data-open-pricing], #creditsButton")?.click();
      }
      return;
    }
    ...
    console.log("🎬 VIDEO kredi düştü:", cost);

    // UI akışı (kredi kesmez)
...
/* =========================================================
   AIVO FINAL ROUTER — open=pricing & auth gate (SINGLE SOURCE)
   - Studio her açılışta URL'yi okur.
   - open=pricing ise:
       - login yoksa -> vitrine auth + return
       - login varsa -> pricing/kredi modalını aç + pack seçer
   ========================================================= */
(function AIVO_Final_OpenPricing_Gate(){
  function getParam(name){
    try { return new URL(location.href).searchParams.get(name) || ""; } catch(e){ return ""; }
  }

  function setUrlWithout(paramsToDelete){
    try{
  ...
  // ✅ EN SAĞLAM login tespiti: Studio UI zaten biliyor.
  // authUser görünürse login var kabul et.
  function isLoggedInStudio(){
    try{
      var authUser = document.getElementById("authUser");
      // authUser varsa ve hidden değilse -> login var
      if (authUser && !authUser.hasAttribute("hidden")) return true;

      // fallback: yaygın storage anahtarları (varsa)
      if (
  ...
  function goVitrinAuth(returnUrl){
    var url = "/?auth=1&return=" + encodeURIComponent(returnUrl);
    location.href = url;
  }

  function openPricing(pack){
    try{
      // pack seç (varsa)
      if (pack && window.AIVO_STORE_V1 && typeof window.AIVO_STORE_V1.setSelectedPack === "function") {
        window.AIVO_STORE_V1.setSelectedPack(pack);
      }
      ...
      // fallback: "Kredi Al" butonunu tetikle
      var btn = document.querySelector('[data-open-pricing], #btnBuyCredits, .js-open-pricing, #btnCreditBuy, #btnPricing');
      if (btn) btn.click();
    }catch(_){}
  }

  function run(){
    var open = (getParam("open") || "").trim();
    if (open !== "pricing") return;

    var pack = (getParam("pack") || "").trim();

    // ✅ Studio login değilse vitrine auth'a gönder
    if (!isLoggedInStudio()){
      goVitrinAuth("/studio.html?open=pricing" + (pack ? ("&pack=" + encodeURIComponent(pack)) : ""));
      return;
    }

    // ✅ login varsa: modalı aç
    openPricing(pack);

    // URL temizle (tekrar açılmasın)
    setUrlWithout(["open","pack"]);
  }
