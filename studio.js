...
// =========================================================
// STRIPE FINALIZER — FINAL / SINGLE SOURCE OF TRUTH
// =========================================================
(function stripeFinalizeWithStore() {
  if (window.__AIVO_STRIPE_FINALIZER_INSTALLED__) return;
  window.__AIVO_STRIPE_FINALIZER_INSTALLED__ = true;
  ...
  try {
    ...
    if (!sessionId) {
      try {
        const u = new URL(location.href);
        const sid = String(u.searchParams.get("session_id") || "").trim();
        if (sid) {
          sessionId = sid;
    ...
    fetch("/api/stripe/verify-session", {
      ...
      .catch(() => {
        localStorage.removeItem(DONE_KEY);
        showToast?.("verify-session hatası", "error");
      });
  } catch (e) {
    console.error("stripeFinalize error", e);
  }
})();
