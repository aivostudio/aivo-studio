...
(function () {
  ...
  function showLocked(msg) {
    ...
    if (lockMsg) {
      lockMsg.textContent =
        msg || "Admin yetkin yok veya giriş bulunamadı.";
    }
  }
  ...
  function getEmailFromStorage() {
    ...
    for (let i = 0; i < keys.length; i++) {
      const v = String(localStorage.getItem(keys[i]) || "")
        .trim()
        .toLowerCase();
  ...
  async function adminAuth() {
    ...
    if (!email) {
      showLocked(
        "Giriş bulunamadı. Önce Studio’da admin e-posta ile giriş yap."
      );
    ...
    try {
      ...
      showLocked(
        "Bu hesap admin allowlist’inde değil (ADMIN_EMAILS)."
      );
      return { ok: false, reason: "not_allowed", email };
    } catch (err) {
      showLocked(
        "Admin auth kontrolü başarısız. /api/admin/auth erişimini kontrol et."
      );
  ...
  adminAuth().then((state) => {
    if (!state.ok) return; // ❌ admin değil → burada DUR

    // =========================
    // 5) Admin UI AKTİF
    // =========================
    const whoEl = $("who");
    if (whoEl && state.email) {
      whoEl.textContent = "Giriş: " + state.email;
    }
    ...
    if (btnGetCredits) {
      btnGetCredits.addEventListener("click", async () => {
        ...
        const email = String($("qEmail")?.value || "")
          .trim()
          .toLowerCase();

        const r = await fetch(
          "/api/admin/credits-get?email=" +
    ...
    if (btnAdjust) {
      btnAdjust.addEventListener("click", async () => {
        ...
        const email = String($("aEmail")?.value || "")
          .trim()
          .toLowerCase();
        const delta = Number(
          String($("aDelta")?.value || "").trim()
        );
        const reason =
          String($("aReason")?.value || "").trim() ||
          "manual_adjust";
