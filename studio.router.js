...
(function () {
  ...
  function setActiveNav(key) {
    ...
    document.querySelectorAll("[data-adfilm-open]").forEach((btn) => {
      const on = key === "adfilm";
      btn.classList.toggle("active", on);
      btn.classList.toggle("is-active", on);
    });
  ...
  async function go(key) {
    ...
    if (key === "adfilm" && typeof window.AIVOEnsureAdFilmAssets === "function") {
      try {
  ...
  function onNavClick(e) {
    const adFilmButton = e.target.closest("[data-adfilm-open]");
    if (adFilmButton) {
      e.preventDefault();
      e.stopPropagation();
      setHash("adfilm");
