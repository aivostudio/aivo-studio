...
async function presignStudioVideo(file) {
  ...
  const filename = file?.name || `studio-video-${Date.now()}.mp4`;
  ...
  const res = await fetch('/api/r2/scan-and-presign', {
    ...
    body: JSON.stringify({
      ...
      personName: '',
      style: '',
      source: 'cartoon_studio_video_upload'
    })
  });
  ...
  if (!res.ok) {
    const msg =
      data?.message ||
      data?.error ||
      (res.status === 403 ? 'media_policy_blocked' : 'studio_video_presign_failed');
    throw new Error(msg);
  }

  if (!data || data.ok === false) {
    throw new Error(data?.message || data?.error || 'studio_video_presign_failed');
  }
...
async function uploadStudioVideoToR2(file) {
  ...
  const filename = file?.name || `studio-video-${Date.now()}.mp4`;
  const { uploadUrl, publicUrl, key, contentType } = await presignStudioVideo(file);
  ...
  if (!scanRes.ok) {
    const msg =
      scanData?.message ||
      scanData?.error ||
      (scanRes.status === 403 ? 'media_policy_blocked' : 'studio_video_scan_upload_failed');
    throw new Error(msg);
  }

  if (!scanData || scanData.ok === false) {
    throw new Error(scanData?.message || scanData?.error || 'studio_video_scan_upload_failed');
  }
...
async function presignStudioLogoFile(file) {
  ...
  const filename = file?.name || `studio-logo-${Date.now()}.png`;
  ...
  const res = await fetch('/api/r2/scan-and-presign', {
    ...
    body: JSON.stringify({
      ...
      personName: '',
      style: '',
      source: 'cartoon_studio_logo_upload'
    })
  });
  ...
  if (!res.ok) {
    const msg =
      data?.message ||
      data?.error ||
      (res.status === 403 ? 'media_policy_blocked' : 'studio_logo_presign_failed');
    throw new Error(msg);
  }

  if (!data || data.ok === false) {
    throw new Error(data?.message || data?.error || 'studio_logo_presign_failed');
  }
...
async function uploadStudioLogoFileToR2(file) {
  ...
  const filename = file?.name || `studio-logo-${Date.now()}.png`;
  const { uploadUrl, publicUrl, key, contentType } = await presignStudioLogoFile(file);
  ...
  if (!scanRes.ok) {
    const msg =
      scanData?.message ||
      scanData?.error ||
      (scanRes.status === 403 ? 'media_policy_blocked' : 'studio_logo_scan_upload_failed');
    throw new Error(msg);
  }

  if (!scanData || scanData.ok === false) {
    throw new Error(scanData?.message || scanData?.error || 'studio_logo_scan_upload_failed');
  }
