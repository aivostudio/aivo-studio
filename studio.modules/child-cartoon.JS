/* =========================================================
   AIVO — CHILD CARTOON UI (TEMP ON social ROUTE)
   - Şimdilik mevcut "social" route'u üstünde çalışır
   - Bu aşama sadece ORTA PANEL UI iskeleti kurar
   - Fal yok
   - DB yok
   - Right panel entegrasyonu yok
   ========================================================= */
...
(function AIVO_CHILD_CARTOON_UI() {
  ...
  const PAGE_KEY = "sm-pack"; // mevcut social route yapısını bozmuyoruz
  ...
  function collectState() {
    const getSingle = (group) => {
      return document.querySelector(`.ccf-group[data-group="${group}"] [data-choice].is-active`)?.getAttribute("data-choice") || "";
    };
  ...
  function bindEvents(root) {
    ...
    root.addEventListener("click", function (e) {
      ...
      if (createBtn) {
        ...
        window.toast?.success?.("Çocuk Çizgifilm arayüzü hazır. Sonraki adımda Fal bağlantısı kurulacak.");
        console.log("[CHILD_CARTOON_UI] current state:", s);
      }
    });
  ...
  function cardButton(label, value, opts) {
    ...
    const attrs = multi
      ? `data-choice-multi data-group-name="${opts.group}" data-choice="${value}" aria-pressed="${opts.active ? "true" : "false"}"`
      : `data-choice-single data-group-name="${opts.group}" data-choice="${value}" aria-pressed="${opts.active ? "true" : "false"}"`;
  ...
  function template() {
    return `
      <section class="page ccf-page is-active" data-page="${PAGE_KEY}" aria-hidden="false">
        <div class="ccf-wrap">
  ...
  function bootWatch() {
    ...
    setInterval(function () {
      if (location.href !== lastHref) {
        ...
        if (location.hash.includes("social") || location.search.includes("to=social")) {
          setTimeout(mount, 30);
        }
      }
    }, 400);
  ...
  console.log("[AIVO] CHILD_CARTOON_UI mounted on social route");
})();
