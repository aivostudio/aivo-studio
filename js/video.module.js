...
(function videoTabSwitchFix(){
  ...
  function bindVideoTabs() {
    ...
    // Tab butonları (sende bu class/attr farklıysa söyle, 2 dk’da uyarlarız)
    const btnText = root.querySelector('[data-vtab="text"], .jsTabText, [data-tab="text"]');
    const btnImage = root.querySelector('[data-vtab="image"], .jsTabImage, [data-tab="image"]');

    // İçerik panelleri
    const viewText = root.querySelector('[data-vview="text"], .jsViewText, [data-view="text"]');
    const viewImage = root.querySelector('[data-vview="image"], .jsViewImage, [data-view="image"]');

    // Eğer selector’lar tutmadıysa bind yapma (sessizce)
    if (!btnText || !btnImage || !viewText || !viewImage) return false;

    // double-bind önle (aynı DOM’a 10 kere bağlanmasın)
    if (root.__videoTabsBound) return true;
    ...
    btnImage.addEventListener("click", (e) => { e.preventDefault(); setMode("image"); });
  ...
  // İlk deneme (defer olsa da bazen route render geç geliyor)
  bindVideoTabs();

  // Route/DOM değişince yeniden dene
  const obs = new MutationObserver(() => bindVideoTabs());
