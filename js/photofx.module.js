(function () {
  ...
  //
  // Prompt-side policy is intentionally narrow:
  // - Normal creative descriptions are allowed.
  // - Only specific artist names and specific political/public figure names
  //   are blocked here.
  // - Image upload scanning remains in /api/r2/scan-upload and is untouched.
  // ------------------------------------------------------------
  ...
  function isPhotoFxPolicyBlocked(raw) {
    ...
    const hasArtistName = PFX_ARTIST_NAME_TERMS.some((term) => {
      const rx = buildPhotoFxPolicyPhraseRegex(term);
      return rx ? rx.test(text) : false;
    });
  ...
  function buildPhotoFxPolicyText(root, form = null) {
    ...
    // Only the user's prompt is checked. Presets, effect controls and
    // system-generated values must not create false policy matches.
