...
(function(){
  "use strict";
  if(window.__AIVO_AD_FILM_PROGRESS_STABILITY_V9__)return;
  window.__AIVO_AD_FILM_PROGRESS_STABILITY_V9__=true;
  ...
  function startedAt(source){var gen=generation(source);return gen.startedAt||gen.createdAt||gen.updatedAt||""}
  ...
  function setButtonActive(on){
    ...
    if(action)action.classList.toggle("is-engine-active",!!on);
  }
  ...
  function renderCompleted(source){
    ...
    node.className="adfilm-engine-status is-visible is-success";node.removeAttribute("data-stage");
  ...
  function renderTerminal(source){
    ...
    node.className="adfilm-engine-status is-visible is-error";node.removeAttribute("data-stage");
  ...
  function render(){var source=project();if(!source)return;if(renderCompleted(source))return;if(renderTerminal(source))return;renderActive(source)}
  ...
  window.AIVOAdFilmProgressUI={render:render};
