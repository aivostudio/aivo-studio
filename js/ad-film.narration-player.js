...
(function AIVO_AD_FILM_NARRATION_PLAYER(){
  "use strict";
  if(window.__AIVO_AD_FILM_NARRATION_PLAYER_V3__)return;
  window.__AIVO_AD_FILM_NARRATION_PLAYER_V3__=true;
  ...
  function removeDownloadControls(host){
    if(!host)return;
    host.querySelectorAll('[data-pa-action="download"]').forEach(function(button){button.remove()});
  }
  ...
  function mount(scope){
    ...
    new MutationObserver(function(){audio.controls=false;audio.removeAttribute('controls');removeDownloadControls(host);sync(scope)}).observe(host,{attributes:true,childList:true,subtree:true,attributeFilter:['src','controls']});
    sync(scope);
  }
