...
(function AIVO_AD_FILM_NARRATION_ENGINE(){
  "use strict";
  if(window.__AIVO_AD_FILM_NARRATION_ENGINE_V1__)return;
  window.__AIVO_AD_FILM_NARRATION_ENGINE_V1__=true;
  ...
  function payload(scope){
    return{
      ...
      text:clean(value(scope,"narrationText","")),
      language:clean(value(scope,"language","tr")),
      voice:clean(value(scope,"voice","warm_female")),
      voiceStyle:clean(value(scope,"voiceStyle","warm")),
      speed:clean(value(scope,"voiceSpeed","balanced")),
      flow:clean(value(scope,"voiceFlow","natural")),
  ...
  function schedulePoll(scope,attempt){
    var controller=controllers.get(scope);if(!controller)return;
    stopPolling(scope);
    controller.timer=setTimeout(function(){poll(scope,attempt||0)},2000);
  }
  async function poll(scope,attempt){
    ...
    try{
      ...
      if(data.status==="COMPLETED"){showAudio(scope,data.audio||{});stopPolling(scope);return}
      if(data.status==="FAILED"){setState(scope,"failed",t("failed"));stopPolling(scope);return}
      setState(scope,data.status==="IN_QUEUE"?"queued":"running",data.status==="IN_QUEUE"?t("queued"):t("running"));
      if((attempt||0)<180)schedulePoll(scope,(attempt||0)+1);else{setState(scope,"failed",t("statusFailed"));stopPolling(scope)}
    }catch(error){console.error("[ADFILM] narration status",error);if((attempt||0)<3)schedulePoll(scope,(attempt||0)+1);else{setState(scope,"failed",t("statusFailed"));notify(t("statusFailed"),"error")}}
  }

  async function create(scope){
    var data=payload(scope);if(!validate(scope,data))return;
    stopPolling(scope);setState(scope,"creating",t("creating"));
    try{
      var result=await request("/api/ad-film/narration/create",{method:"POST",body:JSON.stringify(data)});
      if(result.fallback_used)notify(t("fallback"),"info");
      setState(scope,"queued",t("queued"));schedulePoll(scope,0);
    }catch(error){console.error("[ADFILM] narration create",error);setState(scope,"failed",t("failed"));notify(error&&error.data&&error.data.error==="narration_too_long"?t("tooLong"):t("requestFailed"),"error")}
  }

  async function approve(scope){
    var id=projectId(scope);if(!id)return notify(t("missingProject"),"warning");
    var panel=ensurePanel(scope),button=panel&&panel.querySelector("[data-narration-audio-approve]");if(button)button.disabled=true;
    try{var result=await request("/api/ad-film/narration/status",{method:"POST",body:JSON.stringify({projectId:id,action:"approve"})});showAudio(scope,result.audio||{});notify(t("approved"),"success")}
    catch(error){console.error("[ADFILM] narration approve",error);if(button)button.disabled=false;notify(t("approvalFailed"),"error")}
  }

  function augmentProject(scope,project){
    var next=Object.assign({},project||{}),narration=Object.assign({},next.narration||{});
    narration.voice=clean(value(scope,"voice","warm_female"))||"warm_female";next.narration=narration;return next;
  }
  function patchProjectApi(scope){
    var api=window.AIVOAdFilmProjects;if(!api||api.__narrationVoicePatched)return;
    api.__narrationVoicePatched=true;
    var createProject=api.createProject.bind(api),updateProject=api.updateProject.bind(api);
    api.createProject=function(project){return createProject(augmentProject(root()||scope,project))};
    api.updateProject=function(id,project){return updateProject(id,augmentProject(root()||scope,project))};
  }
  ...
  function applyProject(scope,project){
    ...
    showAudio(scope,project.narration&&project.narration.audio||null);
    var status=clean(project.narrationGeneration&&project.narrationGeneration.status);
    if(status==="queued"||status==="processing"){setState(scope,status==="queued"?"queued":"running",status==="queued"?t("queued"):t("running"));schedulePoll(scope,0)}
  }
  ...
  function translate(scope){
    ...
    var panel=ensurePanel(scope),audio=panel&&panel.querySelector("[data-narration-audio]");
    var state=panel&&panel.dataset.state||"idle";setState(scope,state,t(state));
    var approve=panel&&panel.querySelector("[data-narration-audio-approve]");if(approve)approve.textContent=approve.classList.contains("is-approved")?t("approvedButton"):t("approve");
    var createButton=panel&&panel.querySelector("[data-narration-create]");if(createButton)createButton.textContent=audio&&audio.src?t("regenerate"):t("create");
  }
  ...
  function bind(scope){
    ...
    var controller={timer:null};controllers.set(scope,controller);
    var panel=ensurePanel(scope);
    panel.addEventListener("click",function(event){var createButton=event.target.closest("[data-narration-create]");if(createButton){event.preventDefault();create(scope);return}var approveButton=event.target.closest("[data-narration-audio-approve]");if(approveButton){event.preventDefault();approve(scope)}});
  ...
  document.addEventListener("aivo:adfilm-project-sync",function(event){var scope=root();if(!scope)return;bind(scope);applyProject(scope,event&&event.detail&&event.detail.project||null)});
