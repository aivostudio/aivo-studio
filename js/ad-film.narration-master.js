...
(function AIVO_AD_FILM_NARRATION_MASTER(){
  "use strict";
  if(window.__AIVO_AD_FILM_NARRATION_MASTER_V1__)return;
  window.__AIVO_AD_FILM_NARRATION_MASTER_V1__=true;
  ...
  function signature(source){var audio=source&&source.narration&&source.narration.audio;return[source&&source.id,audio&&audio.url,audio&&audio.createdAt].join("|")}

  async function master(source){
    source=source||project();
    var audio=source&&source.narration&&source.narration.audio;
    if(busy||!source||!source.id||!audio||!clean(audio.url)||audio.mastered===true)return;
    var key=signature(source);if(attempted.has(key))return;attempted.add(key);busy=true;
    var handle=toast(text("Ses profesyonel olarak işleniyor...","Professionally mastering the narration..."),"info");
    try{
      ...
      toast(text("Ses temizlendi ve yayın seviyesinde hazırlandı.","The narration was cleaned and mastered for broadcast."),"success");
    }catch(error){
      if(handle&&typeof handle.dismiss==="function")handle.dismiss();
      console.error("[ADFILM] narration mastering",error);
      attempted.delete(key);
      toast(text("Ses işleme tamamlanamadı. Tekrar denenecek.","Narration mastering could not finish. It will be retried."),"warning");
    }finally{busy=false}
  }
