  ...
  async function presignCartoonReference(file) {
    ...
    const res = await fetch("/api/r2/scan-and-presign", {
      ...
      body: JSON.stringify({
        ...
        source: "cartoon_basic_reference_upload"
      })
    });
  ...
  async function uploadCartoonReferenceToR2(file) {
    ...
    if (!scanRes.ok) {
      const msg =
        scanData?.message ||
        scanData?.error ||
        (scanRes.status === 403 ? "media_policy_blocked" : "cartoon_reference_scan_upload_failed");
      throw new Error(msg);
    }

    if (!scanData || scanData.ok === false) {
      throw new Error(scanData?.message || scanData?.error || "cartoon_reference_scan_upload_failed");
    }
...
async function uploadCartoonAudioToR2(file) {
  ...
  async function presignCartoonLogo(file) {
    ...
    const res = await fetch("/api/r2/scan-and-presign", {
      ...
      body: JSON.stringify({
        ...
        personName: "",
        style: String(state?.style || "").trim(),
        source: "cartoon_basic_logo_upload"
      })
    });
    ...
    if (!res.ok) {
      const msg =
        data?.message ||
        data?.error ||
        (res.status === 403 ? "media_policy_blocked" : "cartoon_logo_presign_failed");
      throw new Error(msg);
    }

    if (!data || data.ok === false) {
      throw new Error(data?.message || data?.error || "cartoon_logo_presign_failed");
    }
  ...
  async function uploadCartoonLogoToR2(file) {
    ...
    const { uploadUrl, publicUrl, key } = await presignCartoonLogo(file);
    ...
    if (!scanRes.ok) {
      const msg =
        scanData?.message ||
        scanData?.error ||
        (scanRes.status === 403 ? "media_policy_blocked" : "cartoon_logo_scan_upload_failed");
      throw new Error(msg);
    }

    if (!scanData || scanData.ok === false) {
      throw new Error(scanData?.message || scanData?.error || "cartoon_logo_scan_upload_failed");
    }
      ...
      const payload = buildBasicPayload();
        ...
        async function tryRefund(reason, extraMeta = {}) {
          ...
          try {
            ...
            console.error("[CARTOON][BASIC] refund failed:", refundErr);
          }
