...
function createPresetCharacterEntry(role, value, label) {
  return {
    key: `preset:${role}:${value}`,
    source: "preset",
    role: role === "main" ? "main" : "helper",
...
function showStoryCharacterLimitAlert() {
 ...
 function normalizeStorySceneDuration(value) {
  ...
  async function presignStoryAudio(file) {
    const contentType = file?.type || "application/octet-stream";
    const filename = file?.name || `story-audio-${Date.now()}.mp3`;
    const storyIdeaText = String(state?.storyIdea || "").trim();
    const extraPromptText = String(state?.extraPrompt || "").trim();
    const styleText = String(state?.style || "").trim();
    const promptText = [storyIdeaText, extraPromptText].filter(Boolean).join(" · ");
    ...
    const res = await fetch("/api/r2/scan-and-presign", {
      ...
      body: JSON.stringify({
        ...
        personName: "",
        style: styleText,
        source: "cartoon_story_audio_upload"
      })
    });
    ...
    if (!res.ok) {
      const msg =
        data?.message ||
        data?.error ||
        (res.status === 403 ? "media_policy_blocked" : "story_audio_presign_failed");
      throw new Error(msg);
    }

    if (!data || data.ok === false) {
      throw new Error(data?.message || data?.error || "story_audio_presign_failed");
    }
  ...
  async function uploadStoryAudioToR2(file) {
    ...
    const { uploadUrl, publicUrl, key } = await presignStoryAudio(file);
    ...
    if (!scanRes.ok) {
      const msg =
        scanData?.message ||
        scanData?.error ||
        (scanRes.status === 403 ? "media_policy_blocked" : "story_audio_scan_upload_failed");
      throw new Error(msg);
    }

    if (!scanData || scanData.ok === false) {
      throw new Error(scanData?.message || scanData?.error || "story_audio_scan_upload_failed");
    }
  ...
  async function presignStoryLogo(file) {
    const contentType = file?.type || "application/octet-stream";
    const filename = file?.name || `story-logo-${Date.now()}.png`;
    const storyIdeaText = String(state?.storyIdea || "").trim();
    const extraPromptText = String(state?.extraPrompt || "").trim();
    const styleText = String(state?.style || "").trim();
    const promptText = [storyIdeaText, extraPromptText].filter(Boolean).join(" · ");
    ...
    const res = await fetch("/api/r2/scan-and-presign", {
      ...
      body: JSON.stringify({
        ...
        personName: "",
        style: styleText,
        source: "cartoon_story_logo_upload"
      })
    });
    ...
    if (!res.ok) {
      const msg =
        data?.message ||
        data?.error ||
        (res.status === 403 ? "media_policy_blocked" : "story_logo_presign_failed");
      throw new Error(msg);
    }

    if (!data || data.ok === false) {
      throw new Error(data?.message || data?.error || "story_logo_presign_failed");
    }
  ...
  async function uploadStoryLogoToR2(file) {
    ...
    const { uploadUrl, publicUrl, key } = await presignStoryLogo(file);
    ...
    if (!scanRes.ok) {
      const msg =
        scanData?.message ||
        scanData?.error ||
        (scanRes.status === 403 ? "media_policy_blocked" : "story_logo_scan_upload_failed");
      throw new Error(msg);
    }

    if (!scanData || scanData.ok === false) {
      throw new Error(scanData?.message || scanData?.error || "story_logo_scan_upload_failed");
    }
