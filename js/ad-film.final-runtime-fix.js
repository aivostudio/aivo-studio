...
(function AIVO_AD_FILM_FINAL_RUNTIME_FIX(){
  ...
  function hideStaleCompleted(scope){
    ...
    var node=status(scope);if(!node||!node.classList.contains("is-success"))return;
  ...
  function scheduleCleanup(scope){
    clearTimeout(cleanupTimer);
    var checks=0;
    function check(){
      checks++;
      hideStaleCompleted(scope||root());
