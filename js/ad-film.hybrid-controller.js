...
(function AIVO_AD_FILM_HYBRID_CONTROLLER(){
  ...
  function projectFinalReady(source){
    ...
    var generationDone=clean(generation.status).toLowerCase()==="completed"&&!!clean(generation.videoUrl)&&generation.awaitingFinalComposite!==true&&generation.avatarWaiting!==true&&generation.sourceOnly!==true;
    var mixed=Number(generation.mixVersion||0)>=4&&!!clean(generation.videoUrl);
    var finalized=clean(finalization.status).toLowerCase()==="completed"&&!!clean(generation.videoUrl);
    var avatarDone=clean(pipeline.status)==="completed"&&!!clean(pipeline.videoUrl)&&!!clean(generation.videoUrl)&&generation.awaitingFinalComposite!==true;
    return mixed||finalized||generationDone||avatarDone;
  }
  function statusFinalReady(data){
    ...
    return status==="completed"&&publicStatus==="COMPLETED"&&!!clean(data&&data.video_url||generation.videoUrl);
  }
  function recoverableAvatarStart(source){
    if(!source||!source.avatar||source.avatar.enabled!==true||!sourceUrl(source)||projectFinalReady(source))return false;
    var pipeline=source.avatar.pipeline;
    if(pipeline&&["motion_queued","motion_processing","lipsync_queued","lipsync_processing","rendering","completed","failed"].indexOf(clean(pipeline.status))>=0)return false;
    var generation=source.generation||{};
    return generation.awaitingFinalComposite===true||generation.avatarWaiting===true||generation.sourceOnly===true||clean(generation.status).toLowerCase()==="processing";
  }
