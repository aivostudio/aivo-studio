...
(function AIVO_AD_FILM_FINAL_OUTPUT_SYNC(){
  "use strict";
  if(window.__AIVO_AD_FILM_FINAL_OUTPUT_SYNC_V4__)return;
  window.__AIVO_AD_FILM_FINAL_OUTPUT_SYNC_V4__=true;
  ...
  function productionId(source){var gen=generation(source),input=gen.input||{};return clean(gen.productionId||input.productionId||source&&source.productionPlan&&source.productionPlan.productionId)}
  function outputId(source){var gen=generation(source);return clean(gen.outputId||gen.requestId)}
  function sourceReady(source){
    var gen=generation(source);
    if(source&&source.avatar&&source.avatar.enabled===true)return /^https:\/\//i.test(clean(gen.sourceVideoUrl));
    return /^https:\/\//i.test(clean(gen.videoUrl||gen.sourceVideoUrl));
  }
  function avatarVideoUrl(source){
    var pipe=pipeline(source);
    var direct=clean(pipe.videoUrl);if(/^https:\/\//i.test(direct))return direct;
    var lipsync=clean(pipe.lipsync&&pipe.lipsync.videoUrl);if(/^https:\/\//i.test(lipsync))return lipsync;
    if(!clean(pipe.lipsyncAudioUrl)){
      var motion=clean(pipe.motion&&pipe.motion.videoUrl||pipe.motionVideoUrl);if(/^https:\/\//i.test(motion))return motion;
    }
    return"";
  }
  function avatarReady(source){return lower(pipeline(source).status)==="completed"&&Boolean(avatarVideoUrl(source))}
  function terminal(source){
    var states=[source&&source.status,generation(source).status,pipeline(source).status,finalization(source).status].map(lower);
  ...
  function isFinalOutput(item){
    return Boolean(item&&clean(item.videoUrl)&&(
      Number(item.mixVersion||0)>=4||item.finalizedAt||item.avatarApplied===true||item.avatarIntegrated===true||item.hybridTimeline===true||clean(item.avatarCompositeMode)
    ));
  }
  function itemProductionId(item){return clean(item&&item.productionId||item&&item.production_id||item&&item.input&&item.input.productionId)}
  function belongsToCurrentProduction(item,source){
    if(!isFinalOutput(item))return false;
    var gen=generation(source),pid=productionId(source),itemPid=itemProductionId(item);
    if(itemPid)return Boolean(pid&&itemPid===pid);
    var id=clean(item&&item.id),requestId=clean(gen.requestId),generationOutputId=clean(gen.outputId);
    if(!id||!(id===requestId||id===generationOutputId))return false;
    var startedAt=Date.parse(gen.startedAt||gen.createdAt||"");
    var completedAt=Date.parse(item&&item.completedAt||item&&item.finalizedAt||item&&item.createdAt||"");
    if(Number.isFinite(startedAt)&&Number.isFinite(completedAt)&&completedAt<startedAt-5000)return false;
    return true;
  }
  function finalOutputs(source){return (Array.isArray(source&&source.outputs)?source.outputs:[]).filter(function(item){return belongsToCurrentProduction(item,source)})}
  ...
  function mount(source,play){
    ...
    if(window.AIVOAdFilmResultControls&&typeof window.AIVOAdFilmResultControls.mount==="function"){
      window.AIVOAdFilmResultControls.mount(url,clean(item.logoUrl),{projectId:projectId(source),outputId:clean(item.id),logoApplied:!!item.logoApplied,play:!!play,source:"final-output"});
    }
  ...
  function clearPartialPreview(source){if(finalOutput(source))return;if(window.AIVOAdFilmResultControls&&typeof window.AIVOAdFilmResultControls.clear==="function")window.AIVOAdFilmResultControls.clear()}
  function schedule(source,delay){var id=projectId(source);if(!id)return;clearTimeout(timers.get(id));timers.set(id,setTimeout(function(){timers.delete(id);run(project()||source)},Math.max(0,Number(delay)||0)))}
  async function run(source){
    ...
    var task=(async function(){
      ...
      try{
        var response=await fetch("/api/ad-film/seedance/finalize",{method:"POST",credentials:"include",cache:"no-store",headers:{"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify({projectId:id,outputId:outputId(source)})});
      ...
      }catch(error){console.warn("[ADFILM] final output recovery",error);schedule(project(),3500)}
  ...
  document.addEventListener("aivo:adfilm-project-sync",function(event){var next=event&&event.detail&&event.detail.project||project();if(next)window.AIVOAdFilmActiveProject=next;schedule(next,80)});
  document.addEventListener("aivo:adfilm-avatar-ready",function(){schedule(project(),50)});
