(() => {
  ...
  function ensureStyle() {
    ...
    style.textContent = `
      ...
      .radio-preview__player{display:grid;grid-template-columns:48px minmax(0,1fr) 38px 38px 38px;gap:9px;align-items:center;padding:10px;border:1px solid rgba(116,84,190,.38);border-radius:14px;background:rgba(9,12,31,.72)}
      ...
      .radio-output-options{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-bottom:10px}.radio-output-options button{display:grid;gap:3px;min-height:84px;padding:12px;border:1px solid rgba(111,85,171,.45);border-radius:14px;background:linear-gradient(145deg,rgba(34,30,67,.96),rgba(21,21,48,.96));color:#b9b2c8;text-align:left;font:inherit;cursor:pointer}
      .radio-output-options b{color:#f5f1fb;font-size:17px}.radio-output-options strong{font-size:13px}.radio-output-options small{color:#8f899e;font-size:10px}.radio-output-options button.is-active{border-color:rgba(255,255,255,.44);background:linear-gradient(120deg,#7447f5,#bf4ed5,#ef58a8);color:#fff;box-shadow:0 10px 24px rgba(129,64,220,.28)}.radio-output-options button.is-active b,.radio-output-options button.is-active small{color:#fff}
      ...
      @media(max-width:900px){.radio-voice-hero{grid-template-columns:54px minmax(0,1fr)}.radio-voice-hero__status{grid-column:1/-1;width:max-content}.radio-preview__player{grid-template-columns:48px minmax(0,1fr) 36px 36px 36px}.adfilm-radio-production__steps{grid-template-columns:1fr}}
    `;
  ...
  function markup() {
    return `
      <section class="adfilm-radio-panel" data-adfilm-radio-panel hidden aria-label="AI Radyo Reklamı Oluştur">
        ...
        <article class="adfilm-radio-card"><div class="adfilm-radio-card__head"><b>02</b><div><h3>Ses Ayarları</h3><p>Müzik ve konuşma seçilen toplam reklam süresine yerleştirilir.</p></div></div><div class="adfilm-radio-two-col"><section><h4>Reklam Süresi</h4><div class="adfilm-radio-durations" data-radio-choice="duration"><button type="button" data-value="15">15 sn</button><button type="button" class="is-active" data-value="30">30 sn</button><button type="button" data-value="45">45 sn</button><button type="button" data-value="60">60 sn</button></div><p>Stable Audio 3 Small Music motoru 120 saniyeye kadar üretimi destekler.</p></section><section><h4>Çıkış Dosyası</h4><div class="radio-output-options" data-radio-choice="outputFormat"><button type="button" class="is-active" data-value="mp3"><b>MP3</b><strong>320 kbps</strong><small>Yayın ve dijital kullanım</small></button><button type="button" data-value="wav"><b>WAV</b><strong>Kayıpsız</strong><small>Stüdyo ve arşiv kalitesi</small></button></div><p>Final tamamlandığında seçtiğin format hazırlanır.</p></section></div></article>
