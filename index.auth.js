...
/* =========================================================
   TOPBAR CREDIT SYNC (Vitrin)
   - Sadece login varsa göster
   - Krediyi localStorage'dan okuyup "Kredi 78" olarak yazar
   - Pricing modal tetikleyicisi: data-open-pricing
   ========================================================= */
(function topbarCreditSync(){
  const elWrap  = document.getElementById("topCredits");
  const elCount = document.getElementById("topCreditCount");
  if (!elWrap || !elCount) return;

  // ✅ Burayı Studio'nun kullandığı key ile aynı yap
  // Eğer emin değilsen: Studio’da hangi key artıp azalıyor onu söyle, birebir aynı yapalım.
  const CREDIT_KEY_CANDIDATES = [
    "aivo_credits",
    "AIVO_CREDITS",
    "credits",
    "userCredits"
  ...
  function readCredits(){
    for (const k of CREDIT_KEY_CANDIDATES){
      const v = localStorage.getItem(k);
      if (v !== null && v !== undefined && v !== "" && !Number.isNaN(parseInt(v, 10))) {
  ...
  function isLoggedIn(){
    // Senin sisteminde authUser görünür/gizli yönetiliyor.
    // Eğer farklı bir auth flag varsa buraya da ekleyebiliriz.
    const authUser = document.getElementById("authUser");
    if (authUser && authUser.hasAttribute("hidden")) return false;

    // Ek güvence: token/uid varsa
    const hasToken =
  ...
  function sync(){
    ...
    elWrap.hidden = false;
    elCount.textContent = String(readCredits());
  }

  // İlk yükleme
  sync();

  // Başka sekmede kredi değişirse anında güncelle
  window.addEventListener("storage", (e) => {
    if (!e) return;
    if (CREDIT_KEY_CANDIDATES.includes(e.key)) sync();
  });

  // İstersen: sayfada kredi tüketimi sonrası manuel tetiklemek için
