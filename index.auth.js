...
/* =========================================================
   ✅ STUDIO HARD GATE RETURN (SAFE / DROP-IN)
   - studio.guard.js index'e: /?open=login&from=<url>
   - burada from'u TARGET_KEY'e yazar (aivo_after_login)
   - open=login ise modal açar
   - mevcut akışları BOZMAZ (sadece eksik parçayı tamamlar)
   ========================================================= */
(function AIVO_HARD_GATE_RETURN_V1(){
  if (window.__AIVO_HARD_GATE_RETURN_V1__) return;
  window.__AIVO_HARD_GATE_RETURN_V1__ = true;

  function safeOpenLogin(){
    try {
      // öncelik: projedeki unified API
      if (typeof window.openAuthModal === "function") { window.openAuthModal("login"); return; }
      if (typeof window.openLoginModal === "function") { window.openLoginModal(); return; }
      // fallback: senin mevcut modal opener
  ...
  function run(){
    try {
      ...
      var from = p.get("from");
      if (from) {
        // sadece aynı origin'e izin ver (open redirect koruması)
        try {
          var u = new URL(from, window.location.origin);
