...
/* ================= AIVO TOPBAR IDENTITY — SINGLE AUTHORITY (GLOBAL) ================= */
(function AIVO_TOPBAR_IDENTITY(){
  ...
  function setInitialFrom(user){
    ...
    // Topbar
    setText('#topUserInitial', ch);
    setText('#topUserName', name || 'Hesap');

    // Panel (varsa)
    setText('#umAvatar', ch);
    setText('#umName', name || 'Hesap');
  ...
  function showAuth(isAuthed){
    ...
    document.documentElement.classList.toggle('is-auth', !!isAuthed);
    document.documentElement.classList.toggle('is-guest', !isAuthed);
  }
  ...
  async function boot(){
    try{
      ...
      if (r.status !== 200){
        showAuth(false);
        setInitialFrom({ email:'', name:'Hesap' }); // guest fallback
        return;
      }
      ...
      showAuth(false);
      setInitialFrom({ email:'', name:'Hesap' });
    }
  }
