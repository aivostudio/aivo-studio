    ...
    return `
      <article class="mobile-cartoon-video-card ${failed ? "is-error" : ""}" data-mobile-cartoon-job="${esc(job.id)}">
        <div class="mobile-cartoon-video-media">
          ${
            ready
                ? `<video class="mobile-cartoon-video" src="${esc(job.videoUrl)}"${posterAttr} playsinline webkit-playsinline preload="metadata"></video>`
              : failed
...
function bindMobileCartoonCharacterActions(){
  ...
  function openMobileCartoonReportSheet(job){
  if (!job) return;

  const oldSheet = document.getElementById("aivoMobileCartoonReportSheet");
  if (oldSheet) {
    oldSheet.remove();
  }

  const reportTitle = cartoonText("İçeriği bildir", "Report content");
  const reportSub = cartoonText(
    "Bu içerikle ilgili sorunu seç.",
    "Select the issue related to this content."
  );
  const detailsLabel = cartoonText("Açıklama", "Details");
  const detailsPlaceholder = cartoonText(
    "İstersen kısa bir açıklama yaz...",
    "Optionally write a short description..."
  );
  const cancelText = cartoonText("Vazgeç", "Cancel");
  const submitText = cartoonText("Raporu gönder", "Submit report");

  const reasons = [
    cartoonText("Rahatsız edici / saldırgan içerik", "Offensive or disturbing content"),
    cartoonText("Nefret / taciz / ayrımcılık", "Hate, harassment or discrimination"),
    cartoonText("Şiddet / tehlikeli içerik", "Violence or dangerous content"),
    cartoonText("Cinsel / uygunsuz içerik", "Sexual or inappropriate content"),
    cartoonText("Telif / marka ihlali", "Copyright or trademark issue"),
    cartoonText("Yanlış / aldatıcı içerik", "Misleading or deceptive content"),
    cartoonText("Diğer", "Other")
  ];

  const sheet = document.createElement("div");
  sheet.id = "aivoMobileCartoonReportSheet";
  sheet.setAttribute("role", "dialog");
  sheet.setAttribute("aria-modal", "true");
  sheet.setAttribute("aria-label", reportTitle);
  sheet.innerHTML = `
    <div data-mobile-cartoon-report-backdrop style="position:fixed;inset:0;z-index:9998;background:rgba(0,0,0,.54);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);"></div>
    <div style="position:fixed;left:14px;right:14px;bottom:86px;z-index:9999;border-radius:28px;padding:18px;background:linear-gradient(135deg,rgba(24,26,42,.98),rgba(18,20,34,.98));border:1px solid rgba(255,255,255,.16);box-shadow:0 24px 70px rgba(0,0,0,.44),inset 0 1px 0 rgba(255,255,255,.12);">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:14px;">
        ...
        <button type="button" data-mobile-cartoon-report-close style="width:38px;height:38px;border-radius:999px;border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.08);color:#fff;font-size:22px;font-weight:900;">×</button>
      </div>
        ...
        ${reasons.map(function(reason, index){
          return `
            <label style="display:flex;align-items:center;gap:10px;min-height:42px;padding:10px 12px;border-radius:16px;background:rgba(255,255,255,.065);border:1px solid rgba(255,255,255,.10);color:rgba(255,255,255,.90);font-size:13px;font-weight:850;">
              <input type="radio" name="mobileCartoonReportReason" value="${esc(reason)}" ${index === 0 ? "checked" : ""} style="width:18px;height:18px;accent-color:#ec4899;">
      ...
      <label style="display:block;margin-bottom:14px;">
        ...
        <textarea data-mobile-cartoon-report-details maxlength="500" placeholder="${esc(detailsPlaceholder)}" style="width:100%;min-height:78px;resize:none;border-radius:16px;padding:12px;border:1px solid rgba(255,255,255,.12);outline:none;background:rgba(0,0,0,.24);color:#fff;font-size:13px;font-weight:750;"></textarea>
      </label>

      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
        <button type="button" data-mobile-cartoon-report-close style="min-height:46px;border-radius:16px;border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.08);color:rgba(255,255,255,.86);font-size:14px;font-weight:950;">${esc(cancelText)}</button>
        <button type="button" data-mobile-cartoon-report-submit style="min-height:46px;border-radius:16px;border:0;background:linear-gradient(135deg,#8b5cf6,#ec4899);color:#fff;font-size:14px;font-weight:950;box-shadow:0 0 24px rgba(236,72,153,.28);">${esc(submitText)}</button>
      </div>
    </div>
  `;
  ...
  sheet.querySelectorAll("[data-mobile-cartoon-report-close], [data-mobile-cartoon-report-backdrop]").forEach(function(el){
    el.addEventListener("click", closeSheet);
  });

  const submitBtn = sheet.querySelector("[data-mobile-cartoon-report-submit]");
  if (submitBtn) {
    submitBtn.addEventListener("click", async function(){
      const checkedReason = sheet.querySelector('input[name="mobileCartoonReportReason"]:checked');
      const detailsEl = sheet.querySelector("[data-mobile-cartoon-report-details]");
      ...
      submitBtn.textContent = cartoonText("Gönderiliyor...", "Sending...");
      ...
      try {
        ...
        const message = cartoonText(
          "Rapor alındı.",
          "Report received."
        );

        setStatus(message);
        mobileCartoonToast("success", message);
      } catch (err) {
        console.error("[MOBILE CARTOON][REPORT ERROR]", err);

        submitBtn.disabled = false;
        submitBtn.textContent = cartoonText("Raporu gönder", "Submit report");
        ...
        function renderCharacterLibraryLoading(){
          ...
          libraryEl.insertAdjacentHTML("afterbegin", `
            <article class="mobile-cartoon-character-card" data-mobile-cartoon-character="${tempCharacterId}">
              <div class="mobile-cartoon-character-thumb">
               <div
  class="mobile-cartoon-character-loading"
  aria-label="${cartoonText("Karakter hazırlanıyor", "Character is preparing")}"
></div>
              </div>
