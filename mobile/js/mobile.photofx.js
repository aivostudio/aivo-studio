...
async function uploadMobilePhotoFxFile(file, kind){
  ...
  payload: row
    ...
    function openMobilePhotoFxReportSheet(job){
    if (!job) return;

    const oldSheet = document.getElementById("aivoMobilePhotoFxReportSheet");
    if (oldSheet) {
      oldSheet.remove();
    }
    ...
    sheet.id = "aivoMobilePhotoFxReportSheet";
    sheet.setAttribute("role", "dialog");
    sheet.setAttribute("aria-modal", "true");
    sheet.setAttribute("aria-label", reportTitle);
    sheet.innerHTML = `
      <div data-mobile-photofx-report-backdrop style="position:fixed;inset:0;z-index:9998;background:rgba(0,0,0,.54);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);"></div>
      <div style="position:fixed;left:14px;right:14px;bottom:86px;z-index:9999;border-radius:28px;padding:18px;background:linear-gradient(135deg,rgba(24,26,42,.98),rgba(18,20,34,.98));border:1px solid rgba(255,255,255,.16);box-shadow:0 24px 70px rgba(0,0,0,.44),inset 0 1px 0 rgba(255,255,255,.12);">
        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:14px;">
          ...
          <button type="button" data-mobile-photofx-report-close style="width:38px;height:38px;border-radius:999px;border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.08);color:#fff;font-size:22px;font-weight:900;">×</button>
        </div>
          ...
          ${reasons.map(function(reason, index){
            return `
              <label style="display:flex;align-items:center;gap:10px;min-height:42px;padding:10px 12px;border-radius:16px;background:rgba(255,255,255,.065);border:1px solid rgba(255,255,255,.10);color:rgba(255,255,255,.90);font-size:13px;font-weight:850;">
                <input type="radio" name="mobilePhotoFxReportReason" value="${esc(reason)}" ${index === 0 ? "checked" : ""} style="width:18px;height:18px;accent-color:#ec4899;">
        ...
        <label style="display:block;margin-bottom:14px;">
          ...
          <textarea data-mobile-photofx-report-details maxlength="500" placeholder="${esc(detailsPlaceholder)}" style="width:100%;min-height:78px;resize:none;border-radius:16px;padding:12px;border:1px solid rgba(255,255,255,.12);outline:none;background:rgba(0,0,0,.24);color:#fff;font-size:13px;font-weight:750;"></textarea>
        </label>

        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
          <button type="button" data-mobile-photofx-report-close style="min-height:46px;border-radius:16px;border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.08);color:rgba(255,255,255,.86);font-size:14px;font-weight:950;">${esc(cancelText)}</button>
          <button type="button" data-mobile-photofx-report-submit style="min-height:46px;border-radius:16px;border:0;background:linear-gradient(135deg,#8b5cf6,#ec4899);color:#fff;font-size:14px;font-weight:950;box-shadow:0 0 24px rgba(236,72,153,.28);">${esc(submitText)}</button>
        </div>
      </div>
    `;
    ...
    sheet.querySelectorAll("[data-mobile-photofx-report-close], [data-mobile-photofx-report-backdrop]").forEach(function(el){
      el.addEventListener("click", closeSheet);
    });

    const submitBtn = sheet.querySelector("[data-mobile-photofx-report-submit]");
    if (submitBtn) {
      submitBtn.addEventListener("click", async function(){
        const checkedReason = sheet.querySelector('input[name="mobilePhotoFxReportReason"]:checked');
        const detailsEl = sheet.querySelector("[data-mobile-photofx-report-details]");
        ...
        try {
          ...
          setStatus(message);
          mobilePhotoFxToast("success", message);
        } catch (err) {
          console.error("[MOBILE PHOTOFX][REPORT ERROR]", err);
     ...
     resultsEl.addEventListener("click", async function(e){
      ...
      if (act === "sound") {
       ...
       if (act === "download") {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error("mobile_photofx_download_failed_" + response.status);
          }
          ...
          console.error("[MOBILE PHOTOFX][DOWNLOAD ERROR]", err);
      ...
      if (act === "share") {
        ...
        return;
         ...
         if (act === "report") {
        openMobilePhotoFxReportSheet(job);
        return;
      }
