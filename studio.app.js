...
// =========================================================
// APP-LAYER: COVER GENERATE (TEK OTORİTE)
// - Prompt boşsa: toast uyarı
// - /api/credits/consume ile 6 kredi düşür
// - Başladı + kredi düştü toast
// - Sonra var olan cover flow’u tetikle (varsa)
// NOT: Kapanışlara/HTML'e dokunma.
// =========================================================
(function AIVO_APP_COVER_GENERATE_SINGLE_AUTH() {
  if (window.__AIVO_APP_COVER_WIRED__) return;
  window.__AIVO_APP_COVER_WIRED__ = true;
  ...
  function getPromptValue() {
    const el =
      document.querySelector("#coverPrompt") ||
      document.querySelector("#imagePrompt") ||
      document.querySelector("textarea[name='coverPrompt']") ||
      document.querySelector("textarea[name='prompt']") ||
      null;
    return (el && (el.value || "").trim()) || "";
  }
  ...
  function setTopCreditsUI(nextCredits) {
    const nodes = [
      document.querySelector("#topCreditCount"),
      document.querySelector("#topCreditCount"), // senin testte bu vardı
      document.querySelector("#topCreditsCount"),
      document.querySelector("[data-credit-count]"),
      document.querySelector("[data-credits]"),
    ].filter(Boolean);
  ...
  // Kapak üretimini başlatan "mevcut" fonksiyonu bulmaya çalış.
  // (generateCover yok -> burada safe fallback zinciri var)
  async function runCoverFlow(btn, prompt) {
    // 1) Eğer projede yeni bir fonksiyon varsa (sen sonradan ekleyebilirsin)
    if (window.AIVO_APP && typeof window.AIVO_APP.runCover === "function") {
      return window.AIVO_APP.runCover({ buttonEl: btn, prompt });
    }

    // 2) Legacy/global flow ismi ihtimalleri
    if (typeof window.AIVO_RUN_COVER_FLOW === "function") {
      return window.AIVO_RUN_COVER_FLOW(btn, prompt);
    }
    if (typeof window.AIVO_RUN_IMAGE_FLOW === "function") {
      return window.AIVO_RUN_IMAGE_FLOW(btn, prompt);
    }
    if (typeof window.AIVO_COVER_GENERATE === "function") {
      return window.AIVO_COVER_GENERATE(btn, prompt);
    }

    // 3) Son çare: custom event (bazı eski sistemler bunu dinliyor olabilir)
    try {
      document.dispatchEvent(new CustomEvent("aivo:cover:generate", { detail: { prompt, buttonEl: btn } }));
      return;
    } catch (_) {}

    // 4) Hiçbiri yoksa log
    console.log("[COVER] generate flow bulunamadı. Kredi tüketildi.", { promptLen: prompt.length });
  }

  document.addEventListener("click", async function(e) {
    const btn = e.target && e.target.closest ? e.target.closest("#coverGenerateBtn") : null;
    if (!btn) return;

    // TEK otorite: burada kilitle
    e.preventDefault();
    e.stopPropagation();

    const prompt = getPromptValue();
    if (!prompt) {
      tError("Kapak açıklaması boş olamaz.");
      return;
    }
    ...
    const r = await consumeCredits(COVER_COST, { promptLen: prompt.length });

    if (!r.ok) {
      btn.disabled = false;
      btn.dataset.loading = "0";

      // MVP: müzik/video ile aynı davranış
      tError("Yetersiz kredi. Kredi satın alman gerekiyor.");
      const to = encodeURIComponent(location.pathname + location.search + location.hash);
      location.href = "/fiyatlandirma.html?from=studio&reason=insufficient_credit&to=" + to;
      return;
    ...
    if (typeof r.credits === "number") setTopCreditsUI(r.credits);
    tOk(`Üretim başladı. ${COVER_COST} kredi düşüldü.`);

    try {
      await runCoverFlow(btn, prompt);
    } catch (err) {
      console.error("[COVER] generate error:", err);
      tError("Kapak üretimi başlatılamadı.");
    } finally {
      btn.disabled = false;
      btn.dataset.loading = "0";
    }
