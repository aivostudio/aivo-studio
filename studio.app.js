...
/* =========================================================
   SM PACK — SINGLE AUTHORITY (FAKE JOB)
   - direct bind (no doc delegated credit logic)
   - prompt gate (auto find)
   - credits consume via /api/credits/consume
   - single toast language (video ile aynı)
   - fake job output
   ========================================================= */
(function AIVO_BIND_SM_PACK_ONCE(){
  // button selector (tercihen id ver)
  const btn =
    document.getElementById("smPackGenerateBtn") ||
    document.querySelector("[data-generate-sm-pack]");

  if (!btn || btn.__aivoBoundSmPack) return;
  btn.__aivoBoundSmPack = true;

  const COST = 5;

  function tError(msg){ window.toast?.error ? window.toast.error(msg) : console.warn(msg); }
  function tWarn(msg){ window.toast?.warning ? window.toast.warning(msg) : console.warn(msg); }
  function tOk(msg){ window.toast?.success ? window.toast.success(msg) : console.log(msg); }
  ...
  function scorePromptCandidate(el) {
    ...
    const hay = [el.id, el.name, el.className,
  ...
  function findPromptElement() {
    ...
    let el = page.querySelector?.("#smPackInput");
    if (el && isVisible(el)) return el;

    el = page.querySelector?.("[data-sm-pack-prompt]");
    if (el && isVisible(el)) return el;
    ...
    if (!candidates.length) {
      document.querySelectorAll?.(".page[data-page='sm-pack'] textarea, .page[data-page='sm-pack'] input, .page[data-page='sm-pack'] [contenteditable='true']")
        .forEach(n => candidates.push(n));
    }
    ...
    for (const c of candidates) {
      const s = scorePromptCandidate(c);
  ...
  function setTopCreditsUI(nextCredits) {
    ...
    nodes.forEach(n => ("value" in n) ? (n.value = String(nextCredits)) : (n.textContent = String(nextCredits)));
  }
  ...
  async function consumeCredits(cost, meta) {
    ...
    const credits = (data && (data.credits ?? data.remainingCredits ?? data.balance)) ?? null;
    return { ok: true, status: res.status, data, credits };
  }
  ...
  async function run(){
    ...
    const job = app.createJob({ type: "SM_PACK", title: "AI Sosyal Medya Paketi", cost: COST });
  ...
  console.log("[SM_PACK] single authority bound:", btn);
})();
