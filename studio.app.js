...
/* =========================================================
   SIDEBAR UNIFY (SAFE)
   - Kopya sidebars'ları SİLMEZ, SADECE GİZLER
   - MutationObserver yok
   - Sadece page link tıkından sonra 1 kez çalışır
   ========================================================= */
(function aivoSidebarUnifySafe(){
  if (window.__aivoSidebarUnifySafeBound) return;
  window.__aivoSidebarUnifySafeBound = true;
  ...
  function unify(){
    ...
    // master: music sayfasındaki sidebar tercih (submenu yapısı daha doğru)
    let master =
      document.querySelector('.page[data-page="music"] .sidebar') ||
      document.querySelector('.page.is-active .sidebar') ||
      document.querySelector('.sidebar');

    if (!master) return;

    // 1) tüm sayfalardaki sidebars'ları gizle
    document.querySelectorAll('.page .sidebar').forEach(sb => {
      sb.style.display = 'none';
      sb.style.pointerEvents = 'none';
    });

    // 2) master'ı aktif sayfada görünür yap
    master.style.display = '';
    master.style.pointerEvents = '';

    // 3) master aktif sayfanın layout'unda değilse, taşı (append/prepend yerine insertBefore)
    const layout = active.querySelector('.layout');
  ...
  // SADECE sayfa linklerine tıklanınca, switchPage sonrası çalıştır
  document.addEventListener('click', function(e){
    const btn = e.target.closest('[data-page-link]');
    if (!btn) return;

    // switchPage DOM'u güncelledikten sonra
    setTimeout(unify, 30);
  }, false);

  // Debug amaçlı manuel tetik
  window.AIVO_UNIFY_SIDEBAR = unify;
})();
