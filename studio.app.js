...
(function AIVO_COVER_SINGLE_AUTH() {
  ...
  function tError(msg){ try{ window.toast?.error?.(msg); }catch(_){} }
  function tOk(msg){ try{ window.toast?.success?.(msg); }catch(_){} }
  ...
  async function generateCoverWithCredit(prompt) {
    // 1) kredi düş
    const r = await consumeCredits(COVER_COST);
    if (!r.ok) {
      tError("Yetersiz kredi. Kredi satın alman gerekiyor.");
      const to = encodeURIComponent(location.pathname + location.search + location.hash);
      location.href = "/fiyatlandirma.html?from=studio&reason=insufficient_credit&to=" + to;
      return { ok:false, error:"no_credits", status:r.status };
    }
    ...
    // 3) UI sync (varsa)
    try { window.AIVO_STORE_V1?.setCredits?.(r.credits); } catch(_) {}
    try { window.AIVO_STORE_V1?.syncCreditsUI?.(); } catch(_) {}

    tOk(`Kapak üretimi başladı. ${COVER_COST} kredi düşüldü.`);
    return { ok:true, ...j, credits:r.credits };
  }
  ...
  document.addEventListener("click", async (e) => {
    ...
    btn.dataset.loading = "1";
    try {
      await generateCoverWithCredit(prompt);
    } finally {
      btn.disabled = false;
      btn.dataset.loading = "0";
    }
