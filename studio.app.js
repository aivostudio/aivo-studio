...
/* =========================================================
   VIDEO — SINGLE AUTHORITY (CREDIT CONSUME + OUTPUT PUSH)
   - Şu an backend video endpoint yok => en azından çıktıyı gösteriyoruz
   - Mock URL sonra gerçek URL ile değişecek
   ========================================================= */
(function AIVO_VIDEO_SINGLE_AUTHORITY(){
  "use strict";
  if (window.__AIVO_VIDEO_SINGLE_AUTHORITY__) return;
  window.__AIVO_VIDEO_SINGLE_AUTHORITY__ = true;

  // 1) GEN_BRIDGE "AIVO_JOBS missing" fix (minimal)
  if (!window.AIVO_JOBS) {
    const _jobs = [];
    window.AIVO_JOBS = {
      list: () => _jobs.slice(),
      pushJob: (job) => { _jobs.unshift(job); return job; },
      push: (job) => { _jobs.unshift(job); return job; },
    };
  }

  // 2) Outputs API yoksa minimal ekle (varsa dokunmaz)
  if (!window.AIVO_OUTPUTS) window.AIVO_OUTPUTS = {};
  if (!window.AIVO_OUTPUTS._list) window.AIVO_OUTPUTS._list = [];
  if (typeof window.AIVO_OUTPUTS.list !== "function") {
    window.AIVO_OUTPUTS.list = () => window.AIVO_OUTPUTS._list.slice();
  }
  if (typeof window.AIVO_OUTPUTS.add !== "function") {
    window.AIVO_OUTPUTS.add = (item) => {
      window.AIVO_OUTPUTS._list.unshift(item);
      try { window.AIVO_OUTPUTS.render?.(); } catch(_) {}
      return item;
    };
  }
  ...
  function getCost() {
    // UI’daki "14 Kredi" chip’i varsa onu okumayı dene
    const chip = [...document.querySelectorAll("*")].find(el => (el.textContent||"").includes("Kredi") && (el.textContent||"").match(/\d+/));
    const m = chip ? (chip.textContent.match(/(\d+)\s*Kredi/i)) : null;
  ...
  function pushVideoOutput({ title, url }) {
    const out = {
    ...
    window.AIVO_OUTPUTS.add(out);
    return out;
  }

  // ŞİMDİLİK mock (sen istersen bunu sonra gerçek URL ile değiştirirsin)
  const MOCK_VIDEO_URL = "https://test-videos.co.uk/vids/bigbuckbunny/mp4/h264/720/Big_Buck_Bunny_720_10s_1MB.mp4";

  // 3) SINGLE AUTHORITY CLICK (capture)
  document.addEventListener("click", async (e) => {
    const btn = e.target?.closest?.("button, [role='button'], a");
    if (!btn) return;

    const t = (btn.textContent || "").trim();
    if (!t.includes("Video Oluştur")) return;
    ...
    try { window.toast?.success?.(`Video kuyruğa alındı • ${cost} kredi düştü`); } catch(_) {}

    // job kaydı (opsiyonel)
    try {
      window.AIVO_JOBS.pushJob?.({ type:"video", status:"queued", createdAt: Date.now() });
    } catch(_) {}

    // OUTPUT: sağ panele video düşür
    pushVideoOutput({ title: "TEST Video", url: MOCK_VIDEO_URL });
