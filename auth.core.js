...
/* =========================================================
   LOGIN AUTO SUBMIT (Enter + Autofill)
   - Form id gerekmez
   - Email+Şifre dolunca login butonuna tıklar
   - Sadece login modal açıkken çalışır
   ========================================================= */
(function bindLoginAutoSubmit() {
  ...
  function findLoginContext() {
    // Modal/backdrop varsa onu baz al; yoksa tüm document
    const root =
      document.querySelector(".login-backdrop, .auth-backdrop, [data-auth-modal], .modal, .dialog") ||
      document;

    const email =
      root.querySelector('input[type="email"], input[name="email"], input[id*="email"]');

    const pass =
      root.querySelector('input[type="password"], input[name="password"], input[id*="pass"], input[id*="sifre"]');

    const btn =
      root.querySelector('[data-action="login"], #btnLogin, #btnLoginUnified, button[type="submit"], .btn-login, .login-submit');

    return { root, email, pass, btn };
  }

  let submitted = false;
  function trySubmit(reason) {
    const { email, pass, btn } = findLoginContext();
    if (!email || !pass || !btn) return false;

    // Safari autofill bazen value'yu geç basar → trim önemli
    ...
    // Butona click yaptır (senin mevcut login handler'ını tetikler)
    btn.click();

    // 2sn sonra hâlâ olmadıysa tekrar izin ver (fail-safe)
  ...
  // 2) Autofill sonrası (Safari) — input/change yakala
  document.addEventListener(
    "input",
    (e) => {
      const t = e.target;
      ...
      // kullanıcı yazarken gereksiz tetiklemeyi azalt
      setTimeout(() => trySubmit("input"), 50);
    },
    true
  );
  ...
  document.addEventListener(
    "change",
    () => setTimeout(() => trySubmit("change"), 50),
    true
  );

  // 3) Modal açılınca kısa bir pencere boyunca dene (autofill gecikmesi için)
  const kick = () => {
    let n = 0;
    const timer = setInterval(() => {
      n++;
      trySubmit("interval");
      if (n >= 20) clearInterval(timer); // ~2s
    }, 100);
  };

  // Sayfa yüklenince ve focus gelince çalıştır
