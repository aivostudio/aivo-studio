...
const CURSOR_KEY = "push:watch:radioad:cursor";
...
function pickCompletedAt(project) {
  return (
    safeText(project?.finalGeneration?.completedAt) ||
    safeText(project?.final?.createdAt) ||
...
function isRecentlyCompleted(project) {
  ...
  const generationStatus = safeText(project?.finalGeneration?.status).toLowerCase();
  const finalUrl = safeText(project?.final?.url);
  ...
  return ageMs >= 0 && ageMs <= RECENT_COMPLETION_MS;
}
...
async function processProject(project) {
  ...
  const finalUrl = safeText(project?.final?.url);
  const finalId = safeText(project?.final?.id) || safeText(project?.id);
  const format = safeText(project?.final?.format || project?.output?.format || "mp3").toLowerCase();
  ...
  try {
    const pushResult = await sendPushToUser({
      userId,
      userUuid,
      titleTr: "📻 Radyo Reklamın Hazır!",
      bodyTr: "AIVO radyo reklamını tamamladı. Dinlemek için dokun.",
      titleEn: "📻 Your Radio Ad Is Ready!",
      bodyEn: "Your AIVO radio ad is ready. Tap to listen.",
      source: "aivo_generation_complete",
      idempotencyKey: `generation-complete:radio-ad:${String(project.id)}:${finalId}`,
      data: {
        project_id: String(project.id),
        final_id: finalId,
        module: "radio-ad",
    ...
    console.error("[push/watch/radio-ad] completion push failed", pushError);
  ...
  try {
    ...
    for (const key of projectKeys) {
      try {
        ...
        console.error("[push/watch/radio-ad] project failed", projectError);
    ...
    console.error("[push/watch/radio-ad] watcher failed", error);

    return res.status(500).json({
      ok: false,
      error: "radio_ad_push_watcher_failed",
