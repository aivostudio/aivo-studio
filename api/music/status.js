...
if (Array.isArray(arr) && arr.length) {
  for (const item of arr) {
    ...
    // TopMediai bazen task detayını item.picked içine koyuyor (audio_url/song_id vs)
    const trackId =
      String(
        item?.picked?.song_id ||
          item?.picked?.id ||
    ...
    if (ready && urlMp3) {
      ...
      outputs.push({
        ...
        meta: {
          ...
          status: st,
          song_id: String(item?.picked?.song_id || item?.song_id || trackId || ""),
        },
      });
    }
  }
