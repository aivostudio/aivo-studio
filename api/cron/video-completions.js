...
const VIDEO_PUSH_START_AT = "2026-08-11T11:30:00.000Z";
...
function pickFinalVideoUrl(row) {
  const meta = row?.meta || {};
  const outputs = Array.isArray(row?.outputs) ? row.outputs : [];

  const explicitFinal = clean(meta.final_video_url);
  if (explicitFinal) return explicitFinal;

  const finalOutput = outputs.find((item) => {
    if (!item || String(item.type || "").toLowerCase() !== "video") return false;

    const variant = clean(item.meta?.variant).toLowerCase();
    const isFinal = item.meta?.is_final === true;
    const url = clean(item.archive_url || item.url);

    return !!url && (isFinal || ["logo_overlay", "mux", "finalized", "final"].includes(variant));
  });

  return clean(finalOutput?.archive_url || finalOutput?.url);
}

function isReady(row) {
  if (!row) return false;
  if (clean(row.status).toLowerCase() !== "done") return false;
  return !!pickFinalVideoUrl(row);
}
...
async function listCandidateJobs(sql) {
  const rows = await sql`
    ...
    where lower(app) = 'video'
      ...
      and created_at >= ${VIDEO_PUSH_START_AT}::timestamptz
...
async function pollVideoStatus(req, jobId) {
  const response = await fetch(
    `${getOrigin(req)}/api/jobs/status?job_id=${encodeURIComponent(jobId)}`,
    {
      ...
      headers: {
        ...
        "x-aivo-internal-source": "video-completion-cron",
      },
      cache: "no-store",
    }
  );
...
function localizedCopy(lang, row) {
  const mode = clean(row?.meta?.mode).toLowerCase();
  const isImageToVideo = mode === "image";

  if (lang === "en") {
    return isImageToVideo
      ? {
          title: "Your image-to-video clip is ready",
          body: "Your AI image-to-video generation is complete. Open AIVO to watch it.",
        }
      : {
          title: "Your AI video is ready",
          body: "Your AI video generation is complete. Open AIVO to watch it.",
        };
  }

  return isImageToVideo
    ? {
        title: "Resimden videon hazır",
        body: "AI resimden video üretimin tamamlandı. İzlemek için AIVO'yu aç.",
      }
    : {
        title: "AI videon hazır",
        body: "AI video üretimin tamamlandı. İzlemek için AIVO'yu aç.",
      };
}

async function sendVideoReadyPush(req, tokenRecord, row) {
  getFirebaseApp();

  const copy = localizedCopy(tokenRecord.lang, row);
  const imageUrl = `${getOrigin(req)}/api/push/video-icon`;
  const videoUrl = pickFinalVideoUrl(row);
  const mode = clean(row?.meta?.mode).toLowerCase() || "video";
...
module.exports = async function handler(req, res) {
  try {
    ...
    for (const candidate of candidates) {
      ...
      try {
        if (!isReady(candidate)) {
          const polled = await pollVideoStatus(req, candidate.id);
        ...
        for (const tokenRecord of tokens) {
          try {
            const messageId = await sendVideoReadyPush(req, tokenRecord, {
              ...row,
        ...
        summary.failed += 1;
        console.error("[video-completions] job failed", candidate?.id, error);
    ...
    console.log("[video-completions]", summary);
    return res.status(200).json({ ok: true, ...summary });
  } catch (error) {
    console.error("[video-completions] fatal", error);
