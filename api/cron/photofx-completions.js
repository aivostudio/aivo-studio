...
const PHOTOFX_PUSH_START_AT = "2026-08-11T11:00:00.000Z";
...
function pickFinalizedVideoUrl(row) {
  ...
  const finalized = outputs.find(
    (item) =>
      ...
      clean(item.url || item.archive_url)
  );

  return clean(meta.final_video_url || finalized?.archive_url || finalized?.url);
}

function isReady(row) {
  if (!row) return false;
  if (clean(row.status).toLowerCase() !== "done") return false;
  if (!clean(row?.meta?.finalized_at)) return false;
...
async function listCandidateJobs(sql) {
  const rows = await sql`
    ...
    where lower(app) = 'photofx'
      ...
      and created_at >= ${PHOTOFX_PUSH_START_AT}::timestamptz
...
async function pollPhotoFxStatus(req, jobId) {
  const response = await fetch(
    `${getOrigin(req)}/api/jobs/status?job_id=${encodeURIComponent(jobId)}`,
    {
      ...
      headers: {
        ...
        "x-aivo-internal-source": "photofx-completion-cron",
      },
      cache: "no-store",
    }
  );
...
async function claimReadyJob(sql, jobId, claimId) {
  ...
  const rows = await sql`
    ...
    where id = ${jobId}::uuid
      ...
      and coalesce(meta->>'finalized_at', '') <> ''
      and coalesce(meta->>'completion_push_sent_at', '') = ''
      and (
...
function localizedCopy(lang) {
  if (lang === "en") {
    return {
      title: "Your PhotoFX clip is ready",
      body: "Your AI PhotoFX video clip is complete. Open AIVO to watch it.",
    };
  }

  return {
    title: "PhotoFX klibin hazır",
    body: "AI Foto Efekt video klibin tamamlandı. İzlemek için AIVO'yu aç.",
  };
}

async function sendPhotoFxReadyPush(req, tokenRecord, row) {
  getFirebaseApp();

  const copy = localizedCopy(tokenRecord.lang);
  const imageUrl = `${getOrigin(req)}/api/push/photofx-icon`;
...
module.exports = async function handler(req, res) {
  try {
    ...
    for (const candidate of candidates) {
      ...
      try {
        if (!isReady(candidate)) {
          const polled = await pollPhotoFxStatus(req, candidate.id);
        ...
        for (const tokenRecord of tokens) {
          try {
            const messageId = await sendPhotoFxReadyPush(req, tokenRecord, {
              ...row,
        ...
        summary.failed += 1;
        console.error("[photofx-completions] job failed", candidate?.id, error);
    ...
    console.log("[photofx-completions]", summary);
    return res.status(200).json({ ok: true, ...summary });
  } catch (error) {
    console.error("[photofx-completions] fatal", error);
