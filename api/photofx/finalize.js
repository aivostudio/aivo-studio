...
// CommonJS
// PhotoFX finalize:
// input: effects_applied / logo_overlay / mux / provider url
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const effectsAppliedOut = outputs.find(
      (o) => isVideo(o) && normVariant(o) === "effects_applied"
    );
    const logoOverlayOut = outputs.find(
      (o) => isVideo(o) && normVariant(o) === "logo_overlay"
    );
    const muxOut = outputs.find(
      (o) => isVideo(o) && normVariant(o) === "mux"
    );
    ...
    const input_url =
      String(meta?.effects_applied_url || "").trim() ||
      pickUrl(effectsAppliedOut) ||
      String(meta?.logo_overlay_url || "").trim() ||
      pickUrl(logoOverlayOut) ||
    ...
    let selected_final_source_variant = "";

    if (
      String(meta?.effects_applied_url || "").trim() ||
      pickUrl(effectsAppliedOut)
    ) {
      selected_final_source_variant = "effects_applied";
    } else if (
      String(meta?.logo_overlay_url || "").trim() ||
      pickUrl(logoOverlayOut)
    ) {
      selected_final_source_variant = "logo_overlay";
    } else if (
      String(meta?.muxed_url || "").trim() ||
      pickUrl(muxOut)
    ) {
      selected_final_source_variant = "mux";
    } else if (
      String(meta?.final_video_url || "").trim() ||
      pickUrl(providerOut)
    ) {
      selected_final_source_variant = "provider";
    } else {
      selected_final_source_variant = "video";
    }
