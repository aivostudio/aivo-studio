...
async function findCartoonJob(sql, requestId) {
...
function pickFinalImageUrl(job) {
  const meta = job?.meta && typeof job.meta === 'object' ? job.meta : {};
  const outputs = Array.isArray(job?.outputs) ? job.outputs : [];

  const direct = safeString(meta.final_image_url || meta.image_url);
  if (direct) return direct;

  const image = outputs.find(
    (item) =>
      String(item?.type || item?.kind || '').toLowerCase() === 'image' &&
      safeString(item?.url || item?.archive_url)
  );

  return image ? safeString(image.url || image.archive_url) : null;
}
...
async function runServerCompletion(req, jobId) {
  ...
  const response = await fetch(url, {
    ...
    headers: {
      ...
      'x-aivo-completion-source': 'fal_cartoon_webhook',
    },
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const sql = neon(conn);
    let job = await findCartoonJob(sql, requestId);
    ...
    job = await findCartoonJob(sql, requestId);

    const dbStatus = String(job?.status || '').toLowerCase();
    const mode = String(job?.meta?.mode || '').toLowerCase();
    const isCharacter = mode === 'character';
    const finalImageUrl = isCharacter ? pickFinalImageUrl(job) : null;
    const finalVideoUrl = isCharacter ? null : pickFinalVideoUrl(job);
    const finalAssetUrl = finalImageUrl || finalVideoUrl;

    if (dbStatus !== 'done' || !finalAssetUrl) {
    ...
    const pushResult = await sendPushToUser({
      ...
      titleTr: isCharacter ? '🖼️ Karakterin Hazır!' : '🎬 Çizgi Filmin Hazır!',
      bodyTr: isCharacter
        ? "AIVO'da oluşturduğun çizgi karakter hazır. Görmek için dokun."
        : "AIVO'da oluşturduğun çizgi film tamamlandı. İzlemek için dokun.",
      titleEn: isCharacter ? '🖼️ Your Character Is Ready!' : '🎬 Your Cartoon Is Ready!',
      bodyEn: isCharacter
        ? 'Your AIVO cartoon character is ready. Tap to view it.'
        : 'Your AIVO cartoon is ready. Tap to watch.',
      source: 'aivo_generation_complete',
      idempotencyKey: `generation-complete:cartoon:${String(job.id)}`,
      imageUrl: isCharacter ? finalImageUrl : null,
      data: {
        job_id: String(job.id),
        module: 'cartoon',
        type: isCharacter ? 'image' : 'video',
        mode: mode || 'basic',
        target: 'productions',
      },
    });
    ...
    console.error('[fal/cartoon/webhook]', error);
