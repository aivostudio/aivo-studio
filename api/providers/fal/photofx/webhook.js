...
async function findPhotoFxJob(sql, requestId) {
...
async function runServerCompletion(req, jobId) {
  ...
  const response = await fetch(url, {
    ...
    headers: {
      ...
      'x-aivo-completion-source': 'fal_photofx_webhook',
    },
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const sql = neon(conn);
    let job = await findPhotoFxJob(sql, requestId);
    ...
    job = await findPhotoFxJob(sql, requestId);
    ...
    const pushResult = await sendPushToUser({
      ...
      titleTr: '✨ PhotoFX’in Hazır!',
      bodyTr: "AIVO'da oluşturduğun PhotoFX tamamlandı. Görmek için dokun.",
      titleEn: '✨ Your PhotoFX Is Ready!',
      bodyEn: 'Your AIVO PhotoFX is ready. Tap to view it.',
      source: 'aivo_generation_complete',
      idempotencyKey: `generation-complete:photofx:${String(job.id)}`,
      data: {
        job_id: String(job.id),
        module: 'photofx',
        type: 'video',
    ...
    console.error('[fal/photofx/webhook]', error);
