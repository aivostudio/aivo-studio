...
function normalizeStatus(value, audioUrl) {
  ...
  if (["COMPLETED", "COMPLETE", "SUCCEEDED", "READY", "DONE"].includes(status)) {
    return "COMPLETED";
  }

  if (["IN_PROGRESS", "PROCESSING", "RUNNING", "STARTED"].includes(status)) {
    return "RUNNING";
  }
...
export default async function handler(req, res) {
  ...
  if (!statusUrl) {
    return res.status(400).json({
      ...
      hint: "A valid https://queue.fal.run status URL is required.",
    });
  }
  ...
  try {
    ...
    const rawStatus = pick(first.data, [
      "status",
      "state",
      "data.status",
      "result.status",
    ...
    if (!audio && normalizedBeforeResult === "COMPLETED") {
      ...
      if (resolvedUrl) {
        ...
        if (second.response.ok) {
          audio = extractAudio(second.data);
        }
      }
    }
