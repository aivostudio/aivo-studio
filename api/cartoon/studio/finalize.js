// /api/cartoon/studio/finalize.js
// CommonJS
// Studio finalize:
...
async function downloadToFile(url, outPath) {
  const r = await fetch(url);
  if (!r.ok) {
    throw new Error(`download_failed:${r.status}:${url}`);
  }
...
async function runFfmpegConcat(listPath, outputPath) {
  await new Promise((resolve, reject) => {
    ...
    p.on("close", (code) => {
      if (code === 0) return resolve();
      reject(new Error(`ffmpeg_concat_failed:${code}:${stderr.slice(-1500)}`));
    });
  });
}
...
async function runFfmpegFaststart(inputPath, outputPath) {
  await new Promise((resolve, reject) => {
    ...
    p.on("close", (code) => {
      if (code === 0) return resolve();
      reject(new Error(`ffmpeg_faststart_failed:${code}:${stderr.slice(-1500)}`));
    });
  });
}
...
async function runFfmpegPreview(inputPath, outputPath, bitrateCfg = {}) {
  ...
  await new Promise((resolve, reject) => {
    ...
    p.on("close", (code) => {
      if (code === 0) return resolve();
      reject(new Error(`ffmpeg_preview_failed:${code}:${stderr.slice(-1500)}`));
    });
  });
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (String(job.app || "").toLowerCase() !== "cartoon") {
    ...
    const scenes = safeArray(meta?.scenes)
      .map((scene, index) => ({
...
const concatBody = normalizedPaths
  ...
  .join("\n");
    ...
    console.error("cartoon studio finalize failed:", e);

    try {
      const body = req.body || {};
      const job_id = safeText(body.job_id);

      if (job_id && isUuidLike(job_id)) {
        const conn = getConn();
        if (conn) {
          const sql = neon(conn);
          await sql`
            ...
            set
              ...
              meta = coalesce(meta, '{}'::jsonb) || ${JSON.stringify({
                studio_export_done: false,
                studio_export_error: String(e?.message || e),
                studio_export_failed_at: new Date().toISOString(),
              })}::jsonb,
              updated_at = now()
            where id = ${job_id}::uuid
          `;
