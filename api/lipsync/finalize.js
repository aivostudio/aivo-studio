// /api/lipsync/finalize.js
// CommonJS
// Lipsync finalize:
// input: provider / final video url
// işlem: ffmpeg remux + faststart + preview encode + poster çıkarma
// output: R2 final mp4 + R2 preview mp4 + R2 poster jpg + DB meta/output patch
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (String(job.app || "").toLowerCase() !== "lipsync") {
      return res.status(400).json({
        ok: false,
        error: "job_not_lipsync",
      });
    }
    ...
    if (existingFinalized && existingPreview && !body.force) {
      return res.status(200).json({
        ...
        poster_url:
          String(meta?.poster_url || meta?.thumbnail_url || meta?.thumb_url || "").trim() ||
          null,
        skipped: true,
    ...
    tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), "aivo-lipsync-finalize-"));
    ...
    const key = `outputs/lipsync/${job_id}/${outputId}.mp4`;
    const previewKey = `outputs/lipsync/${job_id}/${outputId}-preview.mp4`;
    const posterKey = `outputs/lipsync/${job_id}/${outputId}-poster.jpg`;
