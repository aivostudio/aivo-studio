...
function stripAvatarState(project) {
  ...
  return {
    ...
    generation: project.generation ? {
      ...
      avatarUrl: null,
      avatarApplied: false,
      avatarIntegrated: false,
      avatarCompositeMode: null,
      sourceOnly: false,
    } : project.generation,
  };
}
...
function projectPatch(req, user, id) {
  ...
  const sanitized = sanitizeProjectPatch(raw, user, id);
  const withoutAvatar = { ...sanitized };
  delete withoutAvatar.avatar;
  return extendNarrationPatch(raw, withoutAvatar);
}

function mergeWithoutAvatar(current, patch) {
  return stripAvatarState(mergeProject(stripAvatarState(current), patch));
}
...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === "POST") {
      ...
      project = mergeWithoutAvatar(project, projectPatch(req, user, id));
    ...
    if (req.method === "GET") {
      ...
      const cleaned = stripAvatarState(project);
      const changed = JSON.stringify(cleaned) !== JSON.stringify(project);
      const saved = changed ? await saveProject(user, cleaned) : project;
    ...
    if (req.method === "PATCH" || req.method === "PUT") {
      ...
      const saved = await saveProject(user, mergeWithoutAvatar(current, projectPatch(req, user, id)));
      return sendJson(res, 200, { ok: true, project: saved });
    }
