...
function videoUrlFrom(payload) {
  ...
  if (Array.isArray(list)) {
    const item = list.find((entry) =>
      entry && typeof entry.url === "string" && /^https:\/\//i.test(entry.url),
    );
...
function motionQueueExpired(pipeline, result) {
  ...
  return result?.status === "IN_QUEUE" &&
    motion.model !== KLING_STANDARD_I2V &&
    jobAgeMs(motion) >= MOTION_QUEUE_FALLBACK_MS;
}
...
function pipelineLockError(project, pipeline) {
  ...
  if (acceptedDuration && pipelineDuration && acceptedDuration !== pipelineDuration) {
    return "production_duration_mismatch";
  }

  const acceptedRatio = normalizedRatio(
    input.aspectRatio || input.aspect_ratio || project?.productionPlan?.aspectRatio,
  );
  const pipelineRatio = normalizedRatio(pipeline?.aspectRatio);
  if (acceptedRatio && pipelineRatio && acceptedRatio !== pipelineRatio) {
    return "production_aspect_ratio_mismatch";
  }

  const acceptedQuality = clean(
    input.resolution || project?.productionPlan?.quality,
    20,
  ).toLowerCase();
  const pipelineQuality = clean(pipeline?.quality, 20).toLowerCase();
  if (acceptedQuality && pipelineQuality && acceptedQuality !== pipelineQuality) {
    return "production_quality_mismatch";
  }
  ...
  if (
    ...
    pipelineProductionId &&
    generationProductionId !== pipelineProductionId
  ) {
    return "production_lock_mismatch";
  }
  return null;
}
...
function withRecoveredPipelineVideo(pipeline) {
  const videoUrl = pipelineVideoUrl(pipeline);
  if (!videoUrl || clean(pipeline?.videoUrl, 4000) === videoUrl) return pipeline;
  return {
    ...(pipeline || {}),
    status: "completed",
    stage: "completed",
...
function repairCompletedWithoutVideo(pipeline, now) {
  if (clean(pipeline?.status, 80).toLowerCase() !== "completed") return pipeline;
  if (pipelineVideoUrl(pipeline)) return withRecoveredPipelineVideo(pipeline);

  if (pipeline?.lipsync?.requestId) {
    return {
      ...(pipeline || {}),
      ...
      updatedAt: now,
      repairedAt: now,
      repairReason: "completed_without_video_url",
      error: null,
    };
  }

  if (pipeline?.motion?.requestId) {
    return {
      ...(pipeline || {}),
  ...
  return {
    ...
    stage: "waiting_for_seedance",
    completedAt: null,
    updatedAt: now,
    repairedAt: now,
    repairReason: "completed_without_video_or_provider_job",
    error: null,
  };
}
...
function recoverableLocalTimeout(pipeline, generation) {
  ...
  return reason === "avatar_provider_timeout" &&
    Boolean(pipeline?.motion?.requestId || pipeline?.lipsync?.requestId);
}
...
function activeGeneration(project, pipeline, now) {
  return {
    ...
    finalizing: false,
    sourceOnly: true,
    activeAvatarRequestId: clean(
      pipeline?.lipsync?.requestId || pipeline?.motion?.requestId,
      240,
    ) || null,
    error: null,
  };
...
function matchingFinalOutput(project, pipeline) {
  ...
  if (
    ...
    pipelineProductionId &&
    acceptedProductionId !== pipelineProductionId
  ) {
    return null;
  }
  ...
  return outputs.find((item) => {
    if (!isFinalOutput(item)) return false;

    const itemProductionId = outputProductionId(item);
    if (itemProductionId) {
      return Boolean(
        acceptedProductionId && itemProductionId === acceptedProductionId,
      );
    }
    ...
    if (Number.isFinite(generationStartedAt)) {
      const completedAt = Date.parse(
        item?.completedAt || item?.finalizedAt || item?.createdAt || "",
      );
      if (
        Number.isFinite(completedAt) &&
        completedAt < generationStartedAt - 5000
      ) {
        return false;
      }
    }
...
function completedGeneration(project, pipeline, output, now) {
  const generation = project?.generation || {};
  const completedAt = clean(
    output?.completedAt || output?.finalizedAt || generation.completedAt,
    80,
  ) || now;
  return {
    ...generation,
    ...
    avatarUrl: clean(
      output?.avatarVideoUrl ||
        output?.avatarUrl ||
        pipelineVideoUrl(pipeline) ||
        generation.avatarUrl,
      4000,
    ) || null,
    avatarApplied:
      output?.avatarApplied === true ||
      output?.avatarIntegrated === true ||
      generation.avatarApplied === true,
...
async function healCompletedProject(user, project, pipeline, output, now) {
  pipeline = withRecoveredPipelineVideo(pipeline);
  const generation = project?.generation || {};
  const alreadyCanonical =
    clean(project?.status, 40).toLowerCase() === "completed" &&
  ...
  return saveProject(user, {
    ...
    avatar: {
      ...
      pipeline: {
        ...
        status: "completed",
        stage: "completed",
        videoUrl: pipelineVideoUrl(pipeline) || null,
        error: null,
      },
      videoUrl: pipelineVideoUrl(pipeline) || project?.avatar?.videoUrl || null,
    },
    error: null,
    lastError: null,
  });
...
async function completedResponse(res, user, projectId, project, pipeline, output, now) {
  const nextProject = await healCompletedProject(
    user,
    project,
    pipeline,
    output,
  ...
  const nextPipeline = nextProject?.avatar?.pipeline || pipeline;
  return sendJson(res, 200, {
    ...
    avatar_video_url: pipelineVideoUrl(nextPipeline) || null,
    error: null,
...
async function falFetch(url) {
  ...
  return {
    response,
    data: parseJson(await response.text().catch(() => "")),
  };
}

async function readJob(job) {
  if (!job?.model || !job?.requestId) {
    return { status: "FAILED", error: "fal_job_missing" };
  }
  const statusUrl = clean(job?.statusUrl, 1600) ||
    `https://queue.fal.run/${job.model}/requests/${encodeURIComponent(job.requestId)}/status`;
  const responseUrl = clean(job?.responseUrl, 1600) ||
    `https://queue.fal.run/${job.model}/requests/${encodeURIComponent(job.requestId)}`;
  ...
  if (!statusResponse.response.ok) {
    ...
    return {
      ...
      error: providerError(
        statusResponse.data,
        `fal_status_error:${statusResponse.response.status}`,
      ),
      statusUrl,
      responseUrl,
    };
  ...
  const raw = pick(statusResponse.data, [
    "status",
    "state",
    "data.status",
    "result.status",
  ...
  if (!videoUrl && status === "COMPLETED") {
    ...
    if (resultResponse.response.ok) {
      videoUrl = videoUrlFrom(resultResponse.data);
    } else if (
      resultResponse.response.status === 202 ||
      (resultResponse.response.status === 404 && freshJob(job))
    ) {
      return {
        status: "RUNNING",
        videoUrl: null,
      ...
      return {
        status: "FAILED",
        error: providerError(resultResponse.data, "fal_result_error"),
        statusUrl,
        responseUrl,
      };
    }
...
async function submitQueue(model, input) {
  ...
  try {
    ...
    return {
      ...
      requestId,
      statusUrl: clean(
        data?.status_url || data?.statusUrl || data?.urls?.status,
        1600,
      ) || `https://queue.fal.run/${model}/requests/${encodeURIComponent(requestId)}/status`,
      responseUrl: clean(
        data?.response_url || data?.responseUrl || data?.urls?.response,
        1600,
...
function fallbackMotionInput(pipeline) {
  return {
    ...
    negative_prompt:
      "still image, frozen frame, static pose, identity drift, distorted face, extra people, duplicate limbs, warped hands, wrong product, duplicate product, text, logo, watermark, picture-in-picture, low quality, abrupt camera shake",
  };
}
...
async function queueMotionFallback(pipeline) {
  ...
  if (
    pipeline?.directTalkingAvatar === true ||
    current.model === KLING_STANDARD_I2V
  ) {
    return null;
  }
  const job = await submitQueue(
    KLING_STANDARD_I2V,
    fallbackMotionInput(pipeline),
  );
  return {
    ...job,
    provider: "fal",
...
export default async function handler(req, res) {
  try {
    ...
    pipeline = withRecoveredPipelineVideo(pipeline);
    pipeline = repairCompletedWithoutVideo(pipeline, now);
    ...
    const recoveredVideo = pipelineVideoUrl(pipeline);
    if (pipeline.status === "completed" && recoveredVideo) {
      pipeline = {
        ...pipeline,
        status: "completed",
        stage: "completed",
        videoUrl: recoveredVideo,
        error: null,
      };
      const readyProject = {
        ...project,
        ...
        avatar: { ...avatar, pipeline, videoUrl: recoveredVideo },
      };
      return sendJson(res, 200, {
        ok: true,
        projectId,
    ...
    if (pipeline.status === "failed") {
      ...
      if (recoverableLocalTimeout(pipeline, project?.generation)) {
        ...
        const alreadyTerminal =
          ...
          String(project?.generation?.status) === "failed";
        const nextProject = alreadyTerminal
          ? project
          : await saveTerminalProject(
              user,
              project,
              avatar,
              pipeline,
    ...
    if (lockError) {
      const nextProject = await saveTerminalProject(
        user,
        project,
        avatar,
        pipeline,
      ...
      return sendJson(res, 200, {
        ...
        video_url: null,
        error: lockError,
        pipeline: nextProject.avatar.pipeline,
        project: nextProject,
      });
    }

    if (
      pipeline.stage === "waiting_for_seedance" ||
      pipeline.status === "waiting_for_seedance"
    ) {
      const generation = activeGeneration(project, pipeline, now);
    ...
    if (pipeline.stage === "motion") {
      ...
      if (motionQueueExpired(pipeline, result)) {
        ...
        pipeline = {
          ...
          updatedAt: now,
          completedAt: now,
          error: result.error || "avatar_motion_failed",
          motion: {
            ...pipeline.motion,
            error: result.error || "avatar_motion_failed",
          },
      ...
      if (result.status === "FAILED") {
        pipeline = {
          ...
          updatedAt: now,
          completedAt: now,
          error: result.error || "avatar_lipsync_failed",
          lipsync: {
            ...pipeline.lipsync,
            error: result.error || "avatar_lipsync_failed",
          },
        ...
        pipeline = {
          ...pipeline,
          status: queuedStatus(result, "lipsync_queued", "lipsync_processing"),
          updatedAt: now,
          error: null,
          lipsync: {
            ...pipeline.lipsync,
    ...
    const latestFinal = matchingFinalOutput(latestProject, pipeline);
    if (latestFinal) {
      return completedResponse(
        res,
        user,
    ...
    if (pipeline.status === "failed") {
      ...
      const nextProject = await saveTerminalProject(
        ...
        latestAvatar,
        pipeline,
        pipeline.error || "avatar_pipeline_failed",
        now,
      );
      return sendJson(res, 200, {
        ok: true,
        ...
        stage: "failed",
        video_url: null,
        error: nextProject.generation?.error || pipeline.error,
        pipeline: nextProject.avatar.pipeline,
        project: nextProject,
      });
    }

    const safeVideo = pipelineVideoUrl(pipeline);
    if (safeVideo) {
      pipeline = {
        ...pipeline,
        status: "completed",
    ...
    const generation = safeVideo
      ? avatarReadyGeneration(latestProject, pipeline, now)
      : activeGeneration(latestProject, pipeline, now);
    ...
    const nextProject = await saveProject(user, {
      ...
      generation,
      avatar: {
        ...(latestProject.avatar || avatar),
        pipeline,
        videoUrl: safeVideo || null,
      },
    });

    const publicStatus = safeVideo
      ? "COMPLETED"
      : pipeline.status.includes("queued")
        ? "IN_QUEUE"
        : "RUNNING";
    ...
    return sendJson(res, 200, {
      ...
      video_url: safeVideo || null,
      error: null,
      recovered_from_timeout: Boolean(pipeline.timeoutRecoveredAt),
      repaired: Boolean(pipeline.repairedAt),
      pipeline,
      project: nextProject,
    });
  } catch (error) {
    console.error(
      "[ad-film/avatar/pipeline/status-native]",
      error,
      error?.data || "",
    );
