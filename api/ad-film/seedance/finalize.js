...
function outputsOf(project) {
  ...
  if (!outputs.length && (project?.generation?.videoUrl || project?.generation?.sourceVideoUrl)) {
    outputs.push({
      ...
      videoUrl: project.generation.videoUrl || project.generation.sourceVideoUrl,
      sourceVideoUrl: project.generation.sourceVideoUrl || project.generation.videoUrl || null,
...
function avatarWindows(duration) {
  ...
  if (duration <= 10) {
    return [
      [1.0, Math.min(4.2, duration * 0.48)],
      [Math.min(6.0, duration * 0.64), Math.max(6.8, duration - 1.0)],
    ];
  }
  return [
    [1.2, Math.min(5.2, duration * 0.38)],
    [Math.min(8.4, duration * 0.62), Math.min(duration - 1.2, 12.7)],
  ];
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const target =
      outputs.find((item) => clean(item.id) === requestedOutputId) ||
    ...
    const narrationUrl =
      narrationEnabled && narrationAudio?.approved === true
    ...
    const avatarUrl = avatarRequested
      ? clean(avatarPipeline?.videoUrl || project?.avatar?.videoUrl, 4000)
      : "";
    ...
    if (avatarRequested && !avatarUrl) {
      return sendJson(res, 425, {
        ...
        avatar_status: avatarPipeline?.status || "processing",
      });
    }
    ...
    const outputId = clean(
      target?.id || project?.generation?.outputId || project?.generation?.requestId,
      240,
    );
    ...
    if (avatarUrl) {
      filters.push(
        `[${avatarIndex}:v][0:v]scale2ref=w=main_w:h=main_h:flags=fast_bilinear[avatar_scaled][product_base]`,
      );
      filters.push("[product_base]setpts=PTS-STARTPTS[product_timed]");
      filters.push("[avatar_scaled]setpts=PTS-STARTPTS[avatar_timed]");
      filters.push(
        `[product_timed][avatar_timed]overlay=0:0:enable='${avatarEnableExpression(duration)}':eof_action=pass:shortest=0[hybrid]`,
      );
      videoLabel = "hybrid";
    }
    ...
    if (logoUrl) {
      filters.push(
        `[${logoIndex}:v]scale=${width}:-1:flags=fast_bilinear,format=rgba,colorchannelmixer=aa=0.96[logo]`,
      );
      filters.push(
        `[${videoLabel}][logo]overlay=W-w-${margin}:H-h-${margin}:format=auto:shortest=1[vout]`,
      );
      videoLabel = "vout";
    }
    ...
    if (narrationUrl && musicUrl) {
      filters.push(
        `[${narrationIndex}:a]aresample=48000,volume=1.02,adelay=${voiceDelay}|${voiceDelay},apad=pad_dur=30,asplit=2[voice_sc][voice_mix]`,
      );
      filters.push(
        `[${musicIndex}:a]aresample=48000,volume=0.78,afade=t=in:st=0:d=0.18,afade=t=out:st=${fadeOutStart}:d=0.8,apad=pad_dur=30[music]`,
      );
      filters.push(
        "[music][voice_sc]sidechaincompress=threshold=0.045:ratio=3.0:attack=15:release=310:makeup=1[ducked]",
      );
      filters.push(
        "[ducked][voice_mix]amix=inputs=2:duration=longest:dropout_transition=0:normalize=0,alimiter=limit=0.985[aout]",
      );
    } else if (narrationUrl) {
      filters.push(
        `[${narrationIndex}:a]aresample=48000,volume=1.02,apad=pad_dur=30,alimiter=limit=0.985[aout]`,
      );
    } else if (musicUrl) {
      filters.push(
        `[${musicIndex}:a]aresample=48000,volume=0.92,afade=t=in:st=0:d=0.18,afade=t=out:st=${fadeOutStart}:d=0.8,apad=pad_dur=30,alimiter=limit=0.985[aout]`,
      );
    }

    if (!avatarUrl && !logoUrl && filters.length) {
      filters.push("[0:v]null[vsource]");
      videoLabel = "vsource";
    }

    if (filters.length) args.push("-filter_complex", filters.join(";"));
    args.push("-map", filters.length ? `[${videoLabel}]` : "0:v:0");
    ...
    const finalOutput = {
      ...
      musicBedVolume: narrationUrl && musicUrl ? 0.78 : 0.92,
