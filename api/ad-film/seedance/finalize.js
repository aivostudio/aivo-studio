...
function avatarWindows(duration){
  if(duration<=6)return[[Math.min(.7,duration*.15),Math.max(1.8,duration-1.0)]];
  if(duration<=10)return[[1.0,Math.min(4.2,duration*.48)],[Math.min(6.0,duration*.64),Math.max(6.8,duration-1.0)]];
  return[[1.2,Math.min(5.2,duration*.38)],[Math.min(8.4,duration*.62),Math.min(duration-1.2,12.7)]];
}
...
export default async function handler(req,res){
  ...
  try{
    ...
    await download(sourceVideoUrl,inputVideo);if(avatarUrl)await download(avatarUrl,avatarVideo,180*1024*1024);if(logoUrl){await download(logoUrl,originalLogo,20*1024*1024);await prepareTransparentLogo(originalLogo,transparentLogo)}if(narrationUrl)await download(narrationUrl,narrationFile,30*1024*1024);if(musicUrl)await download(musicUrl,musicFile,80*1024*1024);
    const args=["-y","-i",inputVideo];let nextIndex=1,avatarIndex=-1,logoIndex=-1,narrationIndex=-1,musicIndex=-1;if(avatarUrl){avatarIndex=nextIndex++;args.push("-i",avatarVideo)}if(logoUrl){logoIndex=nextIndex++;args.push("-loop","1","-i",transparentLogo)}if(narrationUrl){narrationIndex=nextIndex++;args.push("-i",narrationFile)}if(musicUrl){musicIndex=nextIndex++;args.push("-stream_loop","-1","-i",musicFile)}
    const filters=[];
    let videoLabel="0:v";
    if(avatarUrl){
      filters.push(`[${avatarIndex}:v][0:v]scale2ref=w=main_w:h=main_h:flags=lanczos[avatar_scaled][product_base]`);
      filters.push(`[product_base]setpts=PTS-STARTPTS[product_timed]`);
      filters.push(`[avatar_scaled]setpts=PTS-STARTPTS[avatar_timed]`);
      filters.push(`[product_timed][avatar_timed]overlay=0:0:enable='${avatarEnableExpression(duration)}':eof_action=pass:shortest=0[hybrid]`);
      videoLabel="hybrid";
    }
    if(logoUrl){filters.push(`[${logoIndex}:v]scale=${width}:-1:flags=lanczos,format=rgba,colorchannelmixer=aa=0.96[logo]`);filters.push(`[${videoLabel}][logo]overlay=W-w-${margin}:H-h-${margin}:format=auto:shortest=1[vout]`);videoLabel="vout"}
