...
export default async function handler(req,res){
  ...
  try{
    ...
    /* Professional advertising mix: music opens the film, narration enters
       after a duration-aware intro, and music remains audible beneath speech
       with moderate sidechain ducking instead of disappearing completely. */
    if(narrationUrl&&musicUrl){
      filters.push(`[${narrationIndex}:a]aresample=48000,volume=1.04,adelay=${voiceDelay}|${voiceDelay},apad=pad_dur=60,asplit=2[voice_sc][voice_mix]`);
      filters.push(`[${musicIndex}:a]aresample=48000,highpass=f=35,lowpass=f=16500,volume=0.40,afade=t=in:st=0:d=0.25,afade=t=out:st=${fadeOutStart}:d=0.8,apad=pad_dur=60[music]`);
      filters.push(`[music][voice_sc]sidechaincompress=threshold=0.07:ratio=3.2:attack=20:release=280:makeup=1[ducked]`);
      ...
      filters.push(`[${narrationIndex}:a]aresample=48000,volume=1.04,apad=pad_dur=60,alimiter=limit=0.96[aout]`);
    }else if(musicUrl){
      filters.push(`[${musicIndex}:a]aresample=48000,highpass=f=35,lowpass=f=16500,volume=0.48,afade=t=in:st=0:d=0.25,afade=t=out:st=${fadeOutStart}:d=0.8,apad=pad_dur=60,alimiter=limit=0.96[aout]`);
    }
    ...
    const finalOutput={...(target||{}),id:outputId||target?.id||project?.generation?.requestId,version,sourceVideoUrl,videoUrl:finalUrl,logoUrl:logoUrl||null,logoApplied:!!logoUrl,logoPosition:logoUrl?"bottom-right":null,logoOpacity:logoUrl?0.96:null,narrationUrl:narrationUrl||null,narrationApplied:!!narrationUrl,narrationMastered:narrationAudio?.mastered===true,narrationApprovedAt:narrationAudio?.approvedAt||null,narrationDelayMs:voiceDelay,musicUrl:musicUrl||null,musicApplied:!!musicUrl,musicMode:project?.music?.mode||"auto",musicBedVolume:narrationUrl&&musicUrl?0.40:0.48,mixVersion:MIX_VERSION,finalizedAt:now};
    const nextOutputs=[finalOutput,...outputs.filter(item=>clean(item.id)!==clean(finalOutput.id))].slice(0,30);const nextProject=await saveProject(user,{...project,status:"completed",outputs:nextOutputs,activeOutputId:finalOutput.id,generation:{...(project.generation||{}),status:"completed",outputId:finalOutput.id,sourceVideoUrl,videoUrl:finalUrl,logoUrl:logoUrl||null,logoApplied:!!logoUrl,narrationUrl:narrationUrl||null,narrationApplied:!!narrationUrl,narrationDelayMs:voiceDelay,musicUrl:musicUrl||null,musicApplied:!!musicUrl,musicBedVolume:narrationUrl&&musicUrl?0.40:0.48,mixVersion:MIX_VERSION,finalizedAt:now,completedAt:project?.generation?.completedAt||now,error:null}});
    return sendJson(res,200,{ok:true,projectId,outputId:finalOutput.id,video_url:finalUrl,source_video_url:sourceVideoUrl,logo_url:logoUrl||null,logo_applied:!!logoUrl,narration_url:narrationUrl||null,narration_applied:!!narrationUrl,narration_delay_ms:voiceDelay,music_url:musicUrl||null,music_applied:!!musicUrl,music_bed_volume:narrationUrl&&musicUrl?0.40:0.48,mix_version:MIX_VERSION,project:nextProject,outputs:nextProject.outputs||[],activeOutputId:nextProject.activeOutputId||finalOutput.id});
