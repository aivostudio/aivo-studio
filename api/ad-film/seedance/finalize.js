...
export default async function handler(req,res){
  ...
  try{
    ...
    const logoSatisfied=!logoUrl||target?.logoApplied===true,narrationSatisfied=!narrationEnabled||target?.narrationApplied===true,musicSatisfied=!musicRequired||target?.musicApplied===true,mixSatisfied=Number(target?.mixVersion||0)>=MIX_VERSION;
    if(target?.videoUrl&&logoSatisfied&&narrationSatisfied&&musicSatisfied&&mixSatisfied)return sendJson(res,200,{ok:true,projectId,outputId:target.id,video_url:target.videoUrl,logo_applied:!!logoUrl,narration_applied:!!narrationUrl,music_applied:!!musicUrl,mix_version:target.mixVersion,project});
    ...
    await download(sourceVideoUrl,inputVideo);if(logoUrl){await download(logoUrl,originalLogo,20*1024*1024);await prepareTransparentLogo(originalLogo,transparentLogo)}if(narrationUrl)await download(narrationUrl,narrationFile,30*1024*1024);if(musicUrl)await download(musicUrl,musicFile,80*1024*1024);
    ...
    if(narrationUrl&&musicUrl){
      filters.push(`[${narrationIndex}:a]aresample=48000:filter_size=64:phase_shift=10:cutoff=0.98,volume=1.02,adelay=${voiceDelay}|${voiceDelay},apad=pad_dur=60,asplit=2[voice_sc][voice_mix]`);
      filters.push(`[${musicIndex}:a]aresample=48000:filter_size=64:phase_shift=10:cutoff=0.98,volume=0.78,afade=t=in:st=0:d=0.18,afade=t=out:st=${fadeOutStart}:d=0.8,apad=pad_dur=60[music]`);
      filters.push(`[music][voice_sc]sidechaincompress=threshold=0.045:ratio=3.0:attack=15:release=310:makeup=1[ducked]`);
      filters.push(`[ducked][voice_mix]amix=inputs=2:duration=longest:dropout_transition=0:normalize=0,alimiter=limit=0.985:attack=5:release=80:level=0[aout]`);
    }else if(narrationUrl){
      filters.push(`[${narrationIndex}:a]aresample=48000:filter_size=64:phase_shift=10:cutoff=0.98,volume=1.02,apad=pad_dur=60,alimiter=limit=0.985:attack=5:release=80:level=0[aout]`);
    }else if(musicUrl){
    ...
    if(filters.length)args.push("-filter_complex",filters.join(";"));args.push("-map",logoUrl?"[vout]":"0:v:0");if(narrationUrl||musicUrl)args.push("-map","[aout]");else args.push("-map","0:a:0?");args.push("-c:v","libx264","-preset","ultrafast","-crf","18","-pix_fmt","yuv420p","-c:a","aac","-aac_coder","twoloop","-b:a","320k","-cutoff","20000","-ar","48000","-ac","2","-shortest","-movflags","+faststart",outputVideo);await runFfmpeg(args);
    ...
    const finalOutput={...(target||{}),id:outputId||target?.id||project?.generation?.requestId,version,sourceVideoUrl,videoUrl:finalUrl,logoUrl:logoUrl||null,logoApplied:!!logoUrl,logoPosition:logoUrl?"bottom-right":null,logoOpacity:logoUrl?0.96:null,narrationUrl:narrationUrl||null,narrationApplied:!!narrationUrl,narrationMastered:narrationAudio?.mastered===true,narrationApprovedAt:narrationAudio?.approvedAt||null,narrationDelayMs:voiceDelay,musicUrl:musicUrl||null,musicApplied:!!musicUrl,musicMode:project?.music?.mode||"auto",musicBedVolume:narrationUrl&&musicUrl?0.78:0.92,audioCodec:"aac",audioBitrate:"320k",mixVersion:MIX_VERSION,finalizedAt:now};
    const nextOutputs=[finalOutput,...outputs.filter(item=>clean(item.id)!==clean(finalOutput.id))].slice(0,30);const nextProject=await saveProject(user,{...project,status:"completed",outputs:nextOutputs,activeOutputId:finalOutput.id,generation:{...(project.generation||{}),status:"completed",outputId:finalOutput.id,sourceVideoUrl,videoUrl:finalUrl,logoUrl:logoUrl||null,logoApplied:!!logoUrl,narrationUrl:narrationUrl||null,narrationApplied:!!narrationUrl,narrationDelayMs:voiceDelay,musicUrl:musicUrl||null,musicApplied:!!musicUrl,musicBedVolume:narrationUrl&&musicUrl?0.78:0.92,audioCodec:"aac",audioBitrate:"320k",mixVersion:MIX_VERSION,finalizedAt:now,completedAt:project?.generation?.completedAt||now,error:null}});
    return sendJson(res,200,{ok:true,projectId,outputId:finalOutput.id,video_url:finalUrl,source_video_url:sourceVideoUrl,logo_url:logoUrl||null,logo_applied:!!logoUrl,narration_url:narrationUrl||null,narration_applied:!!narrationUrl,narration_delay_ms:voiceDelay,music_url:musicUrl||null,music_applied:!!musicUrl,music_bed_volume:narrationUrl&&musicUrl?0.78:0.92,audio_codec:"aac",audio_bitrate:"320k",mix_version:MIX_VERSION,project:nextProject,outputs:nextProject.outputs||[],activeOutputId:nextProject.activeOutputId||finalOutput.id});
