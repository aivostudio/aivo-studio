...
export default async function handler(req,res){
  ...
  try{
    ...
    if(narrationUrl)filters.push(`[${narrationIndex}:a]aresample=48000,volume=1.12,apad=pad_dur=60[voice]`);
    if(musicUrl)filters.push(`[${musicIndex}:a]aresample=48000,highpass=f=35,lowpass=f=15500,volume=${narrationUrl?"0.22":"0.48"},apad=pad_dur=60[music]`);
    if(narrationUrl&&musicUrl){filters.push(`[music][voice]sidechaincompress=threshold=0.025:ratio=10:attack=18:release=320:makeup=1[ducked]`);filters.push(`[ducked][voice]amix=inputs=2:duration=longest:dropout_transition=0,alimiter=limit=0.96[aout]`)}else if(narrationUrl)filters.push(`[voice]alimiter=limit=0.96[aout]`);else if(musicUrl)filters.push(`[music]alimiter=limit=0.96[aout]`);
    ...
    const finalOutput={...(target||{}),id:outputId||target?.id||project?.generation?.requestId,version,sourceVideoUrl,videoUrl:finalUrl,logoUrl:logoUrl||null,logoApplied:!!logoUrl,logoPosition:logoUrl?"bottom-right":null,logoOpacity:logoUrl?0.96:null,narrationUrl:narrationUrl||null,narrationApplied:!!narrationUrl,narrationMastered:narrationAudio?.mastered===true,narrationApprovedAt:narrationAudio?.approvedAt||null,musicUrl:musicUrl||null,musicApplied:!!musicUrl,musicMode:project?.music?.mode||"auto",mixVersion:2,finalizedAt:now};
    const nextOutputs=[finalOutput,...outputs.filter(item=>clean(item.id)!==clean(finalOutput.id))].slice(0,30);const nextProject=await saveProject(user,{...project,status:"completed",outputs:nextOutputs,activeOutputId:finalOutput.id,generation:{...(project.generation||{}),status:"completed",outputId:finalOutput.id,sourceVideoUrl,videoUrl:finalUrl,logoUrl:logoUrl||null,logoApplied:!!logoUrl,narrationUrl:narrationUrl||null,narrationApplied:!!narrationUrl,musicUrl:musicUrl||null,musicApplied:!!musicUrl,mixVersion:2,finalizedAt:now,completedAt:project?.generation?.completedAt||now,error:null}});
