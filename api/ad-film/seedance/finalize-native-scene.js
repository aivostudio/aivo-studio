...
const MIX_VERSION = 16;
const FFMPEG_TIMEOUT_MS = 240000;
const SEEDANCE_SHORTFALL_TOLERANCE_SECONDS = 0.25;
const AVATAR_SHORTFALL_TOLERANCE_SECONDS = 1.25;
...
function probeDuration(file) {
  return new Promise((resolve, reject) => {
    ...
    child.on("close", () => {
      ...
      if (!match) return reject(new Error("media_duration_unknown"));
...
function sourceUrl(project) {
  return clean(
    project?.generation?.sourceVideoUrl ||
      ...
      project?.outputs?.[0]?.sourceVideoUrl ||
      project?.outputs?.[0]?.videoUrl,
    4000,
  );
}
...
function gradeFor(project) {
  const category = clean(project?.productionPlan?.productProfile?.category, 80);
  return ["fragrance", "wearable_luxury"].includes(category)
    ? "eq=contrast=1.025:saturation=1.055:brightness=0.004,unsharp=5:5:0.20:5:5:0"
    : "eq=contrast=1.02:saturation=1.035,unsharp=5:5:0.16:5:5:0";
}

function normalizedVideoFilter(inputIndex, size, start, duration, label, grade) {
  const safeStart = Math.max(0, Number(start) || 0);
  const safeDuration = Math.max(0.04, Number(duration) || 0.04);
  return `[${inputIndex}:v]trim=start=${safeStart}:duration=${safeDuration},setpts=PTS-STARTPTS,tpad=stop_mode=clone:stop_duration=${safeDuration},trim=duration=${safeDuration},setpts=PTS-STARTPTS,scale=${size.width}:${size.height}:force_original_aspect_ratio=decrease:flags=lanczos,pad=${size.width}:${size.height}:(ow-iw)/2:(oh-ih)/2:black,setsar=1,fps=30,${grade},format=yuv420p[${label}]`;
}

function avatarRequiredDuration(timeline) {
  const avatarSegments = Array.isArray(timeline?.segments)
    ? timeline.segments.filter((segment) => segment?.source === "avatar")
    : [];
  const maxSourceEnd = avatarSegments.reduce((max, segment) => {
  ...
  return Math.max(Number(timeline?.avatar?.duration) || 0, maxSourceEnd);
}

async function persistTerminalFailure(user, project, code, details = {}) {
  ...
  const next = {
    ...
    generation: {
      ...
      finalizing: false,
      finalization: {
        ...(project?.generation?.finalization || {}),
        status: "failed",
        error: code,
        details,
        completedAt: now,
  ...
  try {
    return await saveProject(user, next);
  } catch (error) {
    console.error("[ad-film/finalize-native-scene] persist terminal failure", error);
    return next;
  }
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (
      !["hybrid-timeline", "native-scene"].includes(avatarPipeline?.compositeMode) ||
      avatarPipeline?.status !== "completed"
    ) {
      return sendJson(res, 425, {
        ok: false,
        error: "native_avatar_video_processing",
        avatar_status: avatarPipeline?.stage || avatarPipeline?.status || "motion",
      });
    }
    ...
    const timeline =
      avatarPipeline.timeline || project?.productionPlan?.timeline || timelineFromProject(project);
    const validation = validateAdFilmTimeline(timeline);
    if (!validation.ok) {
      const failedProject = await persistTerminalFailure(user, project, validation.error);
      return sendJson(res, 409, { ok: false, error: validation.error, project: failedProject });
    }

    const narrationEnabled = project?.narration?.enabled !== false;
    const narrationAudio = project?.narration?.audio;
    const narrationUrl =
      narrationEnabled && narrationAudio?.approved === true ? clean(narrationAudio.url, 4000) : "";
    ...
    if (musicRequired && !musicUrl) {
      return sendJson(res, 409, { ok: false, error: "music_audio_required" });
    }

    const resolution = clean(
      avatarPipeline.quality || project?.generation?.input?.resolution || project?.output?.quality || "1080p",
      20,
    ).toLowerCase();
    const ratio = clean(
      avatarPipeline.aspectRatio ||
        project?.generation?.input?.aspectRatio ||
    ...
    const outputId = clean(
      project?.generation?.outputId ||
        ...
        avatarPipeline?.motion?.requestId ||
        `hybrid-${Date.now()}`,
      240,
    );
    ...
    const [seedanceDuration, avatarDuration] = await Promise.all([
      probeDuration(seedanceFile),
      probeDuration(avatarFile),
    ]);
    const requiredAvatar = avatarRequiredDuration(timeline);
    const avatarShortfall = Math.max(0, requiredAvatar - avatarDuration);
    const seedanceShortfall = Math.max(0, duration - seedanceDuration);

    if (seedanceShortfall > SEEDANCE_SHORTFALL_TOLERANCE_SECONDS) {
      const details = {
        expected_duration: duration,
        actual_duration: Number(seedanceDuration.toFixed(3)),
      };
      const failedProject = await persistTerminalFailure(user, project, "seedance_source_too_short", details);
      return sendJson(res, 409, {
        ok: false,
        error: "seedance_source_too_short",
        ...details,
        project: failedProject,
      });
    }

    if (requiredAvatar && avatarShortfall > AVATAR_SHORTFALL_TOLERANCE_SECONDS) {
      const details = {
        expected_duration: Number(requiredAvatar.toFixed(3)),
        actual_duration: Number(avatarDuration.toFixed(3)),
        allowed_shortfall: AVATAR_SHORTFALL_TOLERANCE_SECONDS,
      };
      const failedProject = await persistTerminalFailure(user, project, "avatar_source_too_short", details);
      return sendJson(res, 409, {
        ok: false,
        error: "avatar_source_too_short",
        ...details,
        project: failedProject,
      });
    }
    ...
    timeline.segments.forEach((segment, index) => {
      ...
      filters.push(normalizedVideoFilter(inputIndex, size, start, Number(segment.duration), label, grade));
    ...
    if (logoUrl) {
      ...
      filters.push(
        `[${logoIndex}:v]scale=${width}:-1:flags=lanczos,format=rgba,colorchannelmixer=aa=0.96[logo]`,
      );
      filters.push(
        `[${videoLabel}][logo]overlay=W-w-${margin}:H-h-${margin}:format=auto:eof_action=pass:shortest=0[vout]`,
      );
      videoLabel = "vout";
    }
    ...
    if (narrationUrl && musicUrl) {
      filters.push(
        `[${narrationIndex}:a]aresample=48000,volume=1.05,apad=pad_dur=${duration + 1},atrim=0:${duration}[voice]`,
      );
      filters.push(
        `[${musicIndex}:a]aresample=48000,volume=0.34,afade=t=in:st=0:d=0.18,afade=t=out:st=${fadeOutStart}:d=0.8,apad=pad_dur=${duration + 1},atrim=0:${duration}[music]`,
      );
      filters.push(
        "[music][voice]amix=inputs=2:duration=longest:dropout_transition=0:normalize=0,alimiter=limit=0.985[aout]",
      );
    } else if (narrationUrl) {
      filters.push(
        `[${narrationIndex}:a]aresample=48000,volume=1.05,apad=pad_dur=${duration + 1},atrim=0:${duration},alimiter=limit=0.985[aout]`,
      );
    } else if (musicUrl) {
      filters.push(
        `[${musicIndex}:a]aresample=48000,volume=0.72,afade=t=in:st=0:d=0.18,afade=t=out:st=${fadeOutStart}:d=0.8,apad=pad_dur=${duration + 1},atrim=0:${duration},alimiter=limit=0.985[aout]`,
      );
    }
    ...
    const now = new Date().toISOString();
    const sourceDurationValidation = {
      seedance: Number(seedanceDuration.toFixed(3)),
      avatar: Number(avatarDuration.toFixed(3)),
      avatarExpected: Number(requiredAvatar.toFixed(3)),
      avatarShortfall: Number(avatarShortfall.toFixed(3)),
      avatarPadded: avatarShortfall > 0.08,
    };
    const finalOutput = {
      id: outputId,
    ...
    const nextOutputs = [
      finalOutput,
      ...previous.filter((item) => clean(item?.id) !== outputId),
    ].slice(0, 30);
    ...
    return sendJson(res, clean(error?.message) === "ffmpeg_timeout" ? 504 : 500, {
      ok: false,
      error: "hybrid_scene_finalize_failed",
      message: clean(error?.message || error, 1200),
      retryable: false,
      video_url: seedanceVideoUrl || null,
    });
  } finally {
