...
try {
  ...
  if (isAtmo && isDone && !hasFinalizedOutput) {
    const baseUrl = getBaseUrl(req);

    fetch(`${baseUrl}/api/atmo/finalize`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        cookie: req.headers.cookie || "",
      ...
      console.warn("AUTO_FINALIZE_FAILED:", e?.message || e);
    });
  }
} catch (e) {
  console.warn("AUTO_FINALIZE_BLOCK_FAILED:", e?.message || e);
    ...
    // =========================
// 4.65) AUTO LOGO OVERLAY REFRESH (PHOTOFX)
// =========================
try {
  const isPhotoFx =
  ...
  const logoUrl =
    String(job?.meta?.logo_url || "").trim() || null;
  ...
  const baseVideoUrl =
    muxedUrlNow || finalUrlNow || providerUrlNow || firstVideoUrl || null;
  ...
  if (
    ...
    try {
      const refreshResp = await fetch(`${baseUrl}/api/photofx/finalize`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          cookie: req.headers.cookie || "",
      ...
      console.log("[AUTO_PHOTOFX_LOGO_REFRESH_RESPONSE]", JSON.stringify({
        job_id,
        ...
        alreadyHasOverlay,
        alreadyDoneFlag,
        currentOverlaySourceUrl,
        muxedUrlNow,
      }, null, 2));
    } catch (e) {
      console.warn("AUTO_PHOTOFX_LOGO_REFRESH_FAILED:", e?.message || e);
    }
  }
} catch (e) {
  console.warn("AUTO_PHOTOFX_LOGO_REFRESH_BLOCK_FAILED:", e?.message || e);
}
// =========================
