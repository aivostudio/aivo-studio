...
module.exports = async (req, res) => {
  try {
    ...
    if (!Number.isFinite(inc) || inc <= 0) return res.status(400).json({ ok: false, error: "amount_invalid" });

    // idempotency: aynı order iki kez işlenmesin
    const orderKey = `orders:applied:${oid}`;
    const already = await redis.get(orderKey);
    if (already) {
      const current = (await redis.get(`credits:${user}`)) ?? 0;
      return res.json({ ok: true, already_applied: true, credits: Number(current) || 0 });
    }

    // önce order işaretle (TTL ile de tutabiliriz)
    await redis.set(orderKey, "1");

    // kredi arttır (atomic)
    const newCredits = await redis.incrby(`credits:${user}`, inc);

    return res.json({ ok: true, already_applied: false, credits: Number(newCredits) || 0 });
  } catch (e) {
