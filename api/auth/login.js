...
export default async function handler(req, res) {
  try {
    ...
    if (banned) return json(res, 403, { ok: false, error: "user_banned" });

    // ✅ iki key’den de oku (kritik fix)
    const u1 = await kvGetJson(`user:${email}`).catch(() => null);
    const u2 = await kvGetJson(`users:${email}`).catch(() => null);
    const user = (u1 && typeof u1 === "object") ? u1 : ((u2 && typeof u2 === "object") ? u2 : null);

    if (!user) return json(res, 401, { ok: false, error: "user_not_found" });
    if (user.disabled === true) return json(res, 403, { ok: false, error: "user_disabled" });
    if (user.verified === false) return json(res, 403, { ok: false, error: "email_not_verified" });

    const hash = user.passwordHash || user.passHash || user.hash || "";
    if (!hash) return json(res, 401, { ok: false, error: "missing_password_hash" });

    const ok = await bcrypt.compare(password, hash).catch(() => false);
    if (!ok) return json(res, 401, { ok: false, error: "invalid_credentials" });

    // ✅ session (cookie) — basit örnek
    const sid = crypto.randomBytes(24).toString("hex");
    await kvSetJson(`sess:${sid}`, { email, createdAt: Date.now() }, { ex: 60 * 60 * 24 * 7 }); // 7 gün

    // HttpOnly cookie
    res.setHeader(
      "Set-Cookie",
      `aivo_sess=${sid}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${60 * 60 * 24 * 7}; Secure`
    );

    return json(res, 200, {
      ok: true,
      user: { email, name: user.name || "", role: user.role || "user" },
    });
  } catch (e) {
