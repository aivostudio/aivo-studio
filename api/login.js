...
module.exports = async (req, res) => {
  try {
    ...
    const token = makeJWT({ sub: email, email, role, iat: now, exp: now + COOKIE_MAX_AGE }, JWT_SECRET);
    ...
    // ✅ HEMEN DÖN (pending biter)
    res.status(200).json({ ok:true, email, role });

    // ✅ Event'i LAZY yükle (import takılmasın)
    setTimeout(() => {
      try {
        ...
        Promise.resolve(onAuthLogin({ userId: email, email, role, ip, userAgent, at: new Date() }))
          .catch(() => {});
      } catch (_) {}
    }, 0);
