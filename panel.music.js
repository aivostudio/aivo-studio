 ...
 if (act === "stems_5_confirm") {
  ...
  window.openStemConfirmModal?.({
    ...
    onConfirm: async () => {
      ...
      await actionStems(card); // ✅ stems işlemini başlat
    }
  });
  return;
}
  ...
  async function poll(cardId){
    ...
    try{
      ...
      const baseId = String(cardId).split("::")[0];
         // 1) Bu kart için audio bulunduysa sadece BU kartı ready yap
      ...
      if (gotAudio) {
        const finalSrc = String(src || playUrl || "").trim();
        const finalOut = String(output_id || existing.output_id || "").trim();
        const finalDur = String(duration || existing.__duration || "").trim();
        ...
        upsertJob({
          ...
          ...(title ? { title } : {}),
          __ui_state: "ready",
          __audio_src: finalSrc,
          output_id: finalOut,
          ...(finalDur ? { __duration: finalDur } : {}),
          __pending_src: "",
          __pending_output_id: "",
          __pending_duration: ""
        });
        ...
        if (!READY_TOASTED.has(cardId)) {
          READY_TOASTED.add(cardId);
      ...
      // 2) Bu kartta audio yoksa processing'de kal ve polling devam et
      upsertJob({
        job_id: cardId,
        id: cardId,
        ...(title ? { title } : {}),
        __ui_state: st === "ready" ? "processing" : st,
        __pending_src: "",
        __pending_output_id: "",
        __pending_duration: ""
      });
    ...
    try{
      ...
      // sonra eski (LS) -> merge
           for (const old of (jobs || [])) {
            ...
            // DB'de yoksa eski item geri eklenmez
          }
